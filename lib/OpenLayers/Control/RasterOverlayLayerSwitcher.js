...
OpenLayers.Control.RasterOverlayLayerSwitcher = 
    ...
    loadContents: function() {
        ...
        this.btn.className = 'button button-raster-layer show-enkel';
        this.btn.id = 'rasterlayeroverlayswitcher-button-toggle';
        this.layersDiv.appendChild(this.btn);
        OpenLayers.Event.observe(this.btn, 'click', 
    ...
    redraw: function() {
        ...
        for (i = 0, len = layers.length; i < len; i++) {
            ...
            if ( ((this.layerGroup && this.layerGroup === layer.layerGroup) || this.layergroup === null) && !baseLayer && (layer.CLASS_NAME === "OpenLayers.Layer.WMTS" || layer.CLASS_NAME === "OpenLayers.Layer.WMS")) {
                ...
                var shortid = layer.shortid.replace(/\./g, '-');
                ...
                inputElem.dataset.shortid = shortid;
                ...
                OpenLayers.Element.addClass(labelSpan, "labelSpan olButton layerTriggerTarget show-" + shortid);
                if (checked) {
                ...
                OpenLayers.Event.observe(labelSpan, 'click', OpenLayers.Function.bind(self.activate, self));
     ...
     */
    updateMap: function() {
      var i, layerEntry, toggleBtn;
      toggleBtn = document.getElementById('rasterlayeroverlayswitcher-button-toggle');
      for (i in this.dataLayers) {
        layerEntry = this.dataLayers[i];
        var isChecked = layerEntry.inputElem.checked;
        var className = 'show-' + layerEntry.inputElem.dataset.shortid;
        if (isChecked) {
            OpenLayers.Element.addClass(toggleBtn, className);
        } else {
            OpenLayers.Element.removeClass(toggleBtn, className);
        }
        layerEntry.layer.setVisibility(isChecked);
      }
    },
    ...
    toggleControls: function (e) {
        OpenLayers.Element.toggleClass(this.div, 'active');
    },
