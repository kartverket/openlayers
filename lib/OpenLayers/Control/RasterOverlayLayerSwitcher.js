...
OpenLayers.Control.RasterOverlayLayerSwitcher =
  OpenLayers.Class(OpenLayers.Control.LayerSwitcher, {
    ...
    loadContents                   : function () {
        ...
        var clickFunction = function (e) {
          ...
          for (var r in lses) {
            if (!!lses[r].toggleWidgetDiv.parentNode) {
              ...
              var layerChooser = document.getElementById("layer_chooser");
              while (layerChooser.firstChild) {
                layerChooser.removeChild(layerChooser.firstChild);
              }
            }
          }
          if (show) {
            OpenLayers.Element.addClass(parent, 'show');
            document.getElementById('layer_chooser').appendChild(
              self.dataLayersDiv
            );
          }
          ...
          $("#layer_chooser").height($("#map_chooser").height());
      ...
      var div = document.getElementById('layerswitcher');
      div.appendChild(this.layersDiv);

      this.map.events.register('rasterLayerChangeRequest', this, this.rasterLayerChangeRequestHandler);
      this.map.events.register('rasterLayerDeactivateLayer', this, this.rasterLayerDeactivateLayerHandler);
    ...
    rasterLayerDeactivateLayerHandler: function (evt) {
      for (i in this.dataLayers) {
        if (this.dataLayers[i].layer.shortid == evt.shortId) {
          OpenLayers.Element.removeClass(this.dataLayers[i].inputElem.parentNode.parentNode.parentNode.parentNode.parentNode, 'show');
        }
      }
    },
    ...
    redraw                         : function () {
      ...
      var self = this,
        trigger = null,
        containsOverlays = false, baseLayer,
        layers,
        layer,
        i,
        len,
      ...
      for (i = 0, len = layers.length; i < len; i++) {
        ...
        if (((this.layerGroup && this.layerGroup === layer.layerGroup) || this.layergroup === null) && !baseLayer && (layer.CLASS_NAME === "OpenLayers.Layer.WMTS" || layer.CLASS_NAME === "OpenLayers.Layer.WMS")) {
          ...
          if (checked) {
            ...
            OpenLayers.Element.addClass(this.layersDiv, "show");
          }
