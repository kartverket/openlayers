...
OpenLayers.Control.RasterOverlayLayerSwitcher = 
    ...
    loadContents: function() {
        var self=this;
        /* Dette er en hack ettersom det ikke funket å sette verdiene via Anathem. */
        this.options.title = 'Land';
        ...
        if (this.options.title) {
            OpenLayers.Element.addClass(self.toggleWidgetDiv, "enkel");
        }
        self.toggleWidgetDiv.id = 'vectorWidgetToggleBtn';
        self.toggleWidgetDiv.tabIndex = 0;
        self.toggleWidgetDiv.innerHTML = OpenLayers.Lang.translate(this.options.title);
     ...
     */
    updateMap: function() {
      var i, layerEntry, vectorWidgetToggleBtn;
      vectorWidgetToggleBtn = document.getElementById('vectorWidgetToggleBtn');
      for (i in this.dataLayers) {
        layerEntry = this.dataLayers[i];
        layerEntry.layer.setVisibility(layerEntry.inputElem.checked);
        if (vectorWidgetToggleBtn !== null) {
          var shortid = layerEntry.layer.shortid.replace(/\./g, '-');
          if (layerEntry.inputElem.checked) {
            vectorWidgetToggleBtn.innerHTML = OpenLayers.Lang.translate(layerEntry.layer.name);
            OpenLayers.Element.addClass(vectorWidgetToggleBtn, shortid);
          } else {
            OpenLayers.Element.removeClass(vectorWidgetToggleBtn, shortid);  
          }
        }
      }
    },
