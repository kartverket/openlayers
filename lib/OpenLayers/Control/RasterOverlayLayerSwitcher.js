...
OpenLayers.Control.RasterOverlayLayerSwitcher = 
    ...
    redraw: function() {
        ...
        for (i = 0, len = layers.length; i < len; i++) {
            ...
            if (layer.displayInLayerSwitcher && !baseLayer && (layer.CLASS_NAME === "OpenLayers.Layer.WMTS" || layer.CLASS_NAME === "OpenLayers.Layer.WMS")) {
    ...
    setLayerVisibilityByShortId: function (shortId) {
        ...
        for (i = 0, len = this.dataLayers.length; i < len; i += 1) {
            ...
            if (layerEntry.layer.shortid !== shortId) {
                ...
                pin = i;
                layerEntry.inputElem.className = "checked";
