...
OpenLayers.Control.RasterOverlayLayerSwitcher = 
    ...
    rasterLayerChangeRequestHandler: function (evt) {
        if (!!evt.layerGroup && (evt.layerGroup != this.layerGroup)) { return; }
      ...
      var i, layerEntry, pin;
        ...
        for (i in this.dataLayers) {
            ...
            if (layerEntry.inputElem.value == button.value) {
                ...
                if (!!layerEntry.layer.minZoom) {
                  if (map.getZoom()<layerEntry.layer.minZoom) {
                    map.zoomTo(layerEntry.layer.minZoom);
                  }
                }
            }
        }
