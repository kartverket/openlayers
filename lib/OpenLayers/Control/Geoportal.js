...
OpenLayers.Control.Geoportal = OpenLayers.Class(OpenLayers.Control, {
  ...
  showControls: function () {
    ...
    html += '<div class="header" id="tabs-container">Koble til tjeneste';
  ...
  getCapabilities: function (url) {
    var reqUrl = NK.functions.getWMSCapabilities(url); //TODO WFS
    ...
    reqUrl.done(function (xml) {
      ...
      $(xml).find('Layer').each(function () {
        ...
        layers.push(title);
        if (!!queryable) {
          NK.functions.addDynamicLayer(title, url, layer, title, false);
        }
      });
