...
OpenLayers.Control.GetURL = 
    ...
    initialize: function(options) {
        ...
        this.title = OpenLayers.Lang.translate('Share map');        
    }, // initialize
	...
	self.map.events.register( 'move', self, self.changeHash );
    ...
    changeHash: function () {        
    	...
    	if ( ! OpenLayers.Element.hasClass( btn, 'active' ) ) return;
    	setTimeout( function() { self.showControls(); }, 500 );
    }, //changeHash
    ...
    showControls: function () {
    	...
    	self.cnt.innerHTML = self.getContent();
    ...
    getContent: function (center) {   
      ...
      // Fix to add www to url when sharing:
      if (url.indexOf('//norgeskart.no') > -1) {
        url = url.replace('//norgeskart.no', '//www.norgeskart.no');
      }
    	if (center && center.lon && center.lat) {	
    	    src = [OpenLayers.Util.preciseRound(center.lon, 0), OpenLayers.Util.preciseRound(center.lat, 0)];
            spl = url.split('/');
            bool = null;
        ...
        safeURL = safeURL.replace(/%5B/g,"%255B").replace(/%5D/g,"%255D");

        html = '<h1 class="h">' + OpenLayers.Lang.translate('Share map') + '</h1>';
...
//        html += '<iframe src="//www.facebook.com/plugins/like.php?href=' + encodeURIComponent(window.location.href) + '&amp;send=false&amp;layout=button_count&amp;width=45&amp;show_faces=false&amp;font=arial&amp;colorscheme=light&amp;action=like&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:45px; height:21px;" allowTransparency="true"></iframe>';
        html += '<a class="share-link facebook" title="' + OpenLayers.Lang.translate('Facebook') + '" href="http://www.facebook.com/sharer.php?u=' + safeURL + '" target="_blank"><span>' + OpenLayers.Lang.translate('Facebook') + '</span>' + OpenLayers.Util.hideFromOldIE('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="140px" height="140px" viewBox="0 0 140 140" preserveAspectRatio="xMidYMid meet" class="icon facebook"><path d="M70.148,0C31.407,0,0,31.407,0,70.149c0,36.744,28.258,66.87,64.225,69.881V86.93h-17.79V64.832h17.79V53.714c0-15.94,11.97-28.909,26.684-28.909h17.79v22.029h-17.79c-2.099,0-4.448,2.854-4.448,6.671v11.327h22.238V86.93H86.461v51.441c30.872-7.356,53.837-35.099,53.837-68.222C140.298,31.407,108.891,0,70.148,0"/></svg>') + '</a>';
