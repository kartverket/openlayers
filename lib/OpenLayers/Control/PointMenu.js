...
OpenLayers.Control.PointMenu = OpenLayers.Class( OpenLayers.Control, {
	...
	*/
     ...
     */
    getEmbedAreaPoints: function ( size ) {
	var self = this, attr = self.attr; 
	...
	var tl = [parseInt(center['x']-(area[0]/2)), parseInt(center['y']-(area[1]/2))];
	var br = [parseInt(center['x']+(area[0]/2)), parseInt(center['y']+(area[1]/2))];
	...
	var event  = attr['clickEvent'], size = [400,300];
	//var points = self.getEmbedAreaPoints( size );
        ...
        controller['handler']['up'](event);

	embed.data = {'centerX':attr['lon'],'centerY':attr['lat'], 'width':size[0],'height':size[1]};
        //self._kiet('', controller['handler']['feature']);
        //controller.events.triggerEvent("featureadded",{'feature':controller['handler']['feature']});
    },
	...
	//embed.embedMarkerPointSelectHandler( event, true );
	//embed.confirmAddedMarker( event, 'kiet' );
    },
    ...
    _kiet: function (text, value) {
        ...
        if (!debug) {
	    var s = 'position:fixed;left:10px;top:10px;height:200px;width:400px;'+
