...
OpenLayers.Control.PointMenu = OpenLayers.Class(OpenLayers.Control, {
  ...
  data           : [
    {
      ...
      'cnt'  : '<div class="loading">Laster...</div>',
      'api'  : [
    ...
    },
    {
      ...
      'cnt'  : '<div class="loading"><span class="touch-instruction">Trykk på kartet</span><span class="no-touch-instruction">Flytt musepekeren over kartet</span> for å måle avstand og retning.</div>'
    },
    {
      'id'   : 'PMshare',
      'title': 'Bruk og del',
  ...
  initialize: function (options) {
    ...
    if (options && options.infoUrl) {
      this.data[0].api[0].url = options.infoUrl;
    }
    if (options && options.forecastUrl) {
      this.data[1].api[0].url = options.forecastUrl;
    }
    if (options && options.sunriseUrl) {
      this.data[1].api[1].url = options.sunriseUrl;
    }
    if (options && options.propertyUrl) {
      this.data[4].api[0].url = options.propertyUrl;
    }
  }, // initialize
  ...
  draw: function () {
    ...
    self.map.events.registerPriority('click', self, function (evt) {
      if (self.doubleClickTimer) {
        window.clearTimeout(self.doubleClickTimer);
        self.doubleClickTimer = null;
      } else {
        self.map.PMtimer = self.doubleClickTimer = window.setTimeout(function () {
          if (!self.justMoved) {
            self.showPointMenu(evt);
          }
          self.doubleClickTimer = null;
        }, 250);
      }
    ...
    self.map.events.registerPriority('touchstart', self, function (evt) {
      if (self.doubleClickTimer) {
        window.clearTimeout(self.doubleClickTimer);
        self.doubleClickTimer = null;
      } else {
        self.map.PMtimerStart = self.doubleClickTimer = window.setTimeout(function () {
          self.showPointMenu(evt);
          self.doubleClickTimer = null;
        }, 250);
      }
    ...
    self.map.events.registerPriority('touchmove', self, function (evt) {
      // reset doubleclicktimer when touch moves within 250 ms (typically panning or zooming)
      if (self.doubleClickTimer) {
        window.clearTimeout(self.doubleClickTimer);
        self.doubleClickTimer = null;
      }
    });
    self.map.events.register('move', self, function () {
      self.justMoved = true;
      if (self.justMovedTimer) {
        window.clearTimeout(self.justMovedTimer);
        self.justMovedTimer = null;
      }
      self.justMovedTimer = window.setTimeout(function () {
        self.justMoved = false;
        self.justMovedTimer = null;
      }, 500);
    });
    self.map.events.register('move', self, self.movePointMenu);
    self.map.events.register('zoom', self, self.movePointMenu);
  ...
  hideControls: function (skipToggle) {
    ...
    if (!skipToggle) {
      OpenLayers.Util.renderToggleToolClick({'self': self}, false);
    }
   ...
   * Method: showControls
   */
  showControls: function (skipToggle) {
   ...
   */
  enable: function () {
    this.map.events.register('click', self, self.showPointMenu);
    this.map.events.registerPriority('touchstart', self, self.showPointMenu);
    this.map.events.register('move', self, self.movePointMenu);
    this.map.events.register('zoom', self, self.movePointMenu);
  }, // enable
   ...
   */
  disable: function () {
    this.map.events.unregister('click', self, self.showPointMenu);
    this.map.events.unregister('touchstart', self, self.showPointMenu);
    this.map.events.unregister('move', self, self.movePointMenu);
    this.map.events.unregister('zoom', self, self.movePointMenu);
  }, // disable
  ...
  showPointMenu: function (e, force, center, simular) {
    ...
    /* Bugfix for click on close btn popup */
    var targ;
    if (!e) {
      var e = window.event;
    }
    ...
    if (targ && targ.hasOwnProperty('nodeType')) {
      if (targ.nodeType == 3) { // defeat Safari bug
        targ = targ.parentNode;
      }
    }

    if (targ && OpenLayers.Element.hasClass(targ, 'olPopupCloseBox')) {
      return false;
    }
    /* /Bugfix */
    ...
    attr['pos'] = getPosition(e);
    attr['center'] = center || self.map.getLonLatFromPixel(attr['pos']);
    attr['lon'] = attr['center'].lon;
    attr['lat'] = attr['center'].lat;
    attr['geometry'] = new OpenLayers.LonLat(attr['lon'], attr['lat']).transform(self.map.getProjectionObject(), new OpenLayers.Projection("EPSG:4326"));
  ...
  displayRuler: function () {
    ...
    self.insertDrawPoint([attr['lon'], attr['lat']], [attr['lon'], attr['lat']], true); /* Swap second attr['lat'] with 15806506 to enable "north" ruler. */
    ...
    self.map.events.registerPriority('touchstart', self, self.measure);
    self.map.events.registerPriority('touchmove', self, self.measure);
    self.toggleMenu(true);
  }, // displayRuler
  ...
  measure: function (e) {
    ...
    if (distance > 1000) {
      ...
      distance2 = parseFloat(distance / 1.852).toFixed(2); // nautiske mil
    } else {
      distance = parseFloat(distance).toFixed(2);
      distance2 = parseFloat(distance / 1852).toFixed(2); // nautiske mil
    }

    /* Calculating degrees between ruler and "north":
    ...
    */
    ...
    if (!!self.map) {
      ...
      for (var l in seaLayers) {
        if (seaLayers[l].getVisibility()) {
          sjoMeasure = '<span class="label">&nbsp;</span><span class="value">' + distance2 + ' <span class="unit">' + unit2 + '</span></span>';
        }
      }
    }
    ...
    widget.innerHTML = '<ul class="measureContent">' +
      ...
      sjoMeasure +
      '</li>' +
      /* Output of degrees between ruler and "north":
  ...
  drawEmbedMarker: function (data) {
    ...
    render = function (e) {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        if (!btn) continue;
        embed.removeMarker((list[i].id + '').replace(/^[a-z\-]+/i, ''));
      }
  ...
  getWeatherInformation: function (text) {
    ...
    /*
     ...
     xml = parser.parseFromString(text.replace(/\r/g,'').replace(/\n/g, ''), "application/xml");
     console.dir(xml);
     } else {
    ...
    if (note['temperature'] && note['symbol']) {
      ...
      src = 'http://api.yr.no/weatherapi/weathericon/1.1/?symbol=' +
        note['symbol']['number'] + ';' +
        (night ? 'is_night=1;' : '') +
  ...
  getSunRiseAndSetInformation: function (text) {
    ...
    sRise = parseInt(sRise.split(':')[0]) - (n / 60) + (':') + sRise.split(':')[1];
    sSet = parseInt(sSet.split(':')[0]) - (n / 60) + (':') + sSet.split(':')[1];
    mRise = parseInt(mRise.split(':')[0]) - (n / 60) + (':') + mRise.split(':')[1];
    mSet = parseInt(mSet.split(':')[0]) - (n / 60) + (':') + mSet.split(':')[1];

    if (parseInt(sRise.split(':')[0]) > 24) sRise = parseInt(sRise.split(':')[0]) - 24 + (':') + sRise.split(':')[1];
    if (parseInt(sSet.split(':')[0]) > 24) sSet = parseInt(sSet.split(':')[0]) - 24 + (':') + sSet.split(':')[1];
    if (parseInt(mRise.split(':')[0]) > 24) mRise = parseInt(mRise.split(':')[0]) - 24 + (':') + mRise.split(':')[1];
    if (parseInt(mSet.split(':')[0]) > 24) mSet = parseInt(mSet.split(':')[0]) - 24 + (':') + mSet.split(':')[1];
  ...
  displayProperty: function (text, type) {
    ...
    keys = [
      {'key': 'kommunenr'},
      /* @TODO: Finn riktig areal for teiger, sjekk med SeEiendom og mattrikelen.
       {
       'key': 'areal',
       'baseUnit': 'm²',
       'units': [
    ...
    for (i = 0; i < keys.length; i += 1) {
      ...
      if (keys[i].units && typeof !isNaN(parseFloat(info[key], 10))) {
        for (j = 0, k = keys[i].units.length; j < k; j += 1) {
          if (keys[i].units[j].limit < info[key]) {
            unitOfMeasure = keys[i].units[j].unit;
            if (keys[i].units[j].limit > 1) {
              unitOfMeasureFactor = keys[i].units[j].limit;
            } else {
              unitOfMeasureFactor = 1;
            }
          }
      ...
      if (label && info[key]) {
        if (keys[i].units && typeof !isNaN(parseFloat(info[key], 10))) {
          row.push('<tr><td>' + label + '</td><td><span title="' + parseFloat(info[key], 10) + ' ' + keys[i].baseUnit + '">' + OpenLayers.Util.preciseRound(parseFloat(info[key], 10) / unitOfMeasureFactor, 2) + ' ' + unitOfMeasure + '</span></td></tr>');
        } else {
    ...
    if (!isNaN(parseInt(info['kommunenr'], 10)) && !isNaN(parseInt(info['gardsnr'], 10)) && !isNaN(parseInt(info['bruksnr'], 10)) && !isNaN(parseInt(info['festenr'], 10)) && !isNaN(parseInt(info['seksjonsnr'], 10))) {
  ...
  displayInformation: function (text) {
    ...
    if (data['placename'] && data['placename']['value'] && data['placename']['value'].replace(/[ \t]+/g, '')) {
      out.push('<li class="data placename">' +
        ...
        ((data['stedsnummer'] && data['stedsnummer']['value']) ?
        '<a class="value external-link" href="http://faktaark.statkart.no/?stedsnummer=' + data['stedsnummer']['value'] + '">' + data['placename']['value'] + '</a>' :
        '<span class="value">' + data['placename']['value'] + '</span>') +
        (data['placename1'] && data['placename1']['value'] ? '<span class="value">' + data['placename1']['value'] + '</span>' : '') +
        (data['placename2'] && data['placename2']['value'] ? '<span class="value">' + data['placename2']['value'] + '</span>' : '') +              
        '</li>'
      );
    }
    // terrain
    ...
    if (data['elevation'] && data['elevation']['value'] && !isNaN(parseInt(data['elevation']['value'], 10))) {
      out.push('<li class="data elevation"><span class="label">H&oslash;yde: (ansl&aring;tt ved interpolering)</span>' +
    ...
    var coordinateSystemSelector = trf.generateCoordinateSystemsList('coordinate-system-selector', OpenLayers.Lang.translate('Coordinate system:'), crsChoice);
    coordinateSystemSelector.select.value = crsChoice;
    coordinateSystemSelector.select.setAttribute('onchange', "map.getControlsByClass('OpenLayers.Control.Transformations')[0].setCoordinateCookie(this, true)");
    ...
    eastLabel = document.createElement('dt');
    coordinates.appendChild(eastLabel);

    var eastElement = document.createElement('dd');
    eastElement.setAttribute('class', 'value east');
    eastElement.innerHTML = '&nbsp;';
    coordinates.appendChild(eastElement);

    northLabel = document.createElement('dt');
    coordinates.appendChild(northLabel);

    if ((crsChoice == "what3words") || (crsChoice == "Geohash")) {
      northLabel.innerHTML = '';
      eastLabel.innerHTML = '';
    } else if ((crsChoice == "84") || (crsChoice == "50")) {
      northLabel.innerHTML = OpenLayers.Lang.translate('Latitude') + ':';
      eastLabel.innerHTML = OpenLayers.Lang.translate('Longitude') + ':';
    } else {
    ...
    trf.transformData({ost: attr.lon, nord: attr.lat, sosiKoordSys: 23, resSosiKoordSys: crsChoice}, trf.specialOut);
    ...
    OpenLayers.Event.observe(coordinateSystemSelector.select, 'change', function (e) {
      ...
      if ((outSystem == 'what3words') || (outSystem == 'Geohash')) {
        eastLabel.innerHTML = '';
        northLabel.innerHTML = '';
      } else if ((outSystem == "84") || (outSystem == "50")) {
        northLabel.innerHTML = OpenLayers.Lang.translate('Latitude') + ':';
        eastLabel.innerHTML = OpenLayers.Lang.translate('Longitude') + ':';
      } else {
        eastLabel.innerHTML = OpenLayers.Lang.translate('East coordinate') + ':';
  ...
  deleteModuleData: function () {
    ...
    if (attr['UseMapView'] && attr['UseMapView'].deleteStepData) {
      ...
      attr['UseMapView'] = null;
      attr['embedMarkerComment'] = null;
    }

    if (attr['onMeasure']) {
      this.map.events.unregister('touchstart', self, self.measure);
      this.map.events.unregister('touchmove', self, self.measure);

      var exist = attr['draw'] ? (attr['draw']['vector'] || []) : [];
      for (var i = 0; i < exist.length; i++) {
        map.removeLayer(exist[i]);
      }
      attr['onMeasure'] = false;
      attr['draw'] = null;
    }
  },
