...
OpenLayers.Control.PointMenu = 
	...
	*/
     ...
     * Method: createRegExp
     */    
    createRegExp : function( text, g, i, b, f, e, r ) {
	if ( text == '*' ) { return /.*/; }
	...
	var note = {}, out = [], pin = [], reg = [
	   self.createRegExp( 'from="'+ct+'" to="'+ct+'"', true, true, false, false ), 
	   self.createRegExp( 'from="'+cf+'" to="'+ct+'"', true, true, false, false )    
	];
	...
	if ( note['temperature'] ) {	   
		...
		'<span class="value">'+(note['temperature']['value'] || '')+'</span>' +
	...
	// humidity
	if ( note['humidity'] ) {
	    var unit = note['humidity']['unit'] || '';	    
	    out.push( 
	      '<li class="humidity data">' +
                '<span class="label">humidity</span>'+
		'<span class="value">'+(note['humidity']['value'] || '')+'</span>' +
		'<span class="unit '+unit+'">'+unit+'</span>' +
	      '</li>'
	    );
	}
	...
	if ( note['precipitation'] ) {
	    var unit = note['precipitation']['unit'] || '';	    
	    var min  = (note['precipitation']['minvalue'] || '0') + ' ' + unit;	    
	    var max  = (note['precipitation']['mixvalue'] || '0') + ' ' + unit;	    
	    out.push( 
	      '<li class="precipitation data">' +
	...
	widget.innerHTML = '<ul id="weatherWrapper" class="'+style+'">'+out.join('')+'</ul>';
    }, // displayWeather
