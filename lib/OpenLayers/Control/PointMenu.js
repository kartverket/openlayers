...
OpenLayers.Control.PointMenu = OpenLayers.Class( OpenLayers.Control, {
    ...
    attr : { 'ajax':[], 'stroke': {'strokeColor':'#666666','strokeWidth':2} },
    data : [
	{'id'   : 'PMinformation',
    ...
    movePointMenu: function (e) {
		...
		d = self.map.getPixelFromLonLat(attr['center']);
		l = 'left:' + d['x'] + 'px;';
		t = 'top:' + d['y'] + 'px;';
    ...
    showPointMenu: function (e) {
		...
		attr['clickEvent'] = e;
		attr['center'] = self.map.getLonLatFromPixel(e.xy);
		...
		//self.insertDrawPoint( [attr['lon'],attr['lat']], [30,0], true );
		//return self.map.addLayers( attr['draw']['vector'] );
		/*
		var kiet = 
    ...
    clickOnPointMenuWidget : function(e) {
		...
		if (key === 'goBack' ) {
		    index = parseInt(menu.getAttribute('data-view')) || 0;
    ...
    displayWeather : function( text ) {
		...
		sun     = self.getSunRiseAndSetInformation( text );
		weather = self.getWeatherInformation( text );
    ...
    displayInformation : function(text) {
		...
		getViewData = function (key, label) {
		    return data[key] && data[key]['value'] ? ('<li class="data ' + key + '">' + '<span class="label">' + (label || '') + '</span>' + '<span class="value">' + data[key]['value'] + '</span>' + '</li>') : '';
		};
    ...
    pointMenuEndHandler : function (e) {
		...
		attr['pointMenuEndHandlerTImeout'] = setTimeout( 
			function() {
				var main,
					menu,
					close,
			    ...
			    main   = attr['main'];
			    menu = attr['menu'];
			    close  = OpenLayers.Element.hasClass(main, 'onClose');
			    onOpen = OpenLayers.Element.hasClass(main, 'onOpen');
			    callback = attr['menuEndCallback'];
    ...
    _kiet: function (text, value) {
        var debug = document.getElementById('debugWidget');
        var v = '', d = new Date();
	
        if (!debug) {
	    var s = 'position:fixed;right:10px;top:10px;height:200px;width:400px;'+
		'overflow:scroll;background-color:white;color:black;'+
		'z-index:10000;font-size:11px';
	    debug = document.createElement('div');
	    debug.setAttribute('id', 'debugWidget');
	    debug.setAttribute('style', s);
