...
OpenLayers.Control.PointMenu = OpenLayers.Class( OpenLayers.Control, {
	...
	*/
	...
	var self = this, attr = self.attr, embed = attr['UseMapView'];
	if ( ! embed ) return;
	...
	var parent = (main.getAttribute('class') || '')
	    .match( /(^|\s+)by([\w\_\-]+)/gi );
	if ( parent ) {
	...
	if ( ! attr['onMeasure'] || ! e ) return;
	
	var map = self.map, center = map.getLonLatFromPixel(e.xy);
	if ( ! center ) return;
	...
	var unit = 'm', distance = attr['draw']['point'][1][0].distanceTo( 
	    attr['draw']['point'][1][1] 
	);
	
	if ( distance > 1000 ) {
	    var temp = distance / 1000;
	    unit = 'km', distance = parseFloat( temp ).toFixed( 2 );
	}else { distance = parseFloat( distance ).toFixed( 2 ); }
	

	var p1 = map.getPixelFromLonLat(attr['center']);
	var p2 = map.getPixelFromLonLat(center);
	var dx = p2.x-p1.x, dy = p2.y-p1.y;
	var angle = Math.atan2(dx,dy); 
	var degree = (360-(angle*180/Math.PI)-90)-90;
	...
	var start  = new OpenLayers.Geometry.Point(from[0], from[1]);
	var end    = new OpenLayers.Geometry.Point(to[0], to[1]);
    ...
    clickOnPointMenuWidget : function(e) {
		...
		if (OpenLayers.Element.hasClass(target, 'external-link')) {
			if (target.href) {
				e.preventDefault();
				e.stopPropagation();
	...
	*/
	...
	var elements = embed['stepSpecificElements'] || {};
	var heading  = elements['heading'], note = elements['instructions'];
	if ( ! note ) return;

	if ( heading ) heading.innerHTML = 'Mark&oslash;r';
    ...
    displayProperty : function( text, type ) {
        ...
        var self = this, attr = self.attr, widget = attr['widget'];
        var info = {}, row = [], keys = [
            'kommunenr','areal','gardsnr','bruksnr','festenr','seksjonsnr','eiendomstype', 'matrikkelnr'
        ];

        for ( var i=0; i<keys.length; i++ ) {
            var key = keys[i], splited = text.split(
                OpenLayers.Util.createRegExp('<matrikkel:'+key+'>',true,true)
            ) || [];
        ...
        for ( var i=0; i<keys.length; i++ ) {
            var key = keys[i], value = info[key] || '&nbsp;';
            var label = key.charAt(0).toUpperCase() + key.slice(1);
            row.push('<tr><td>'+label+'</td><td>'+value+'</td></tr>');
        }
	...
	*/
    ...
    displayInformation : function(text) {
		...
		// coordinate
		epsg = [Math.round(attr['lon'] * 100) / 100,Math.round(attr['lat'] * 100) / 100];
		for (i = 0; i < epsg.length; i++) {
		    d = 10;
		    t = epsg[i] + (epsg[i] % 1 ? ((epsg[i] * 10) % 1 ? '' : '0') : '.00');	    
		    
		    if (t.length < d) {
				a = new Array((d - t.length) + 1);
				t = a.join('0') + t;
		    }
		    epsg[i] = t;
		}
		...
		slc = document.getElementById('transformations-output-coordinate-system');
