...
OpenLayers.Control.Embed =
  OpenLayers.Class(OpenLayers.Control, {
    ...
    insertContent: function (holder) {
      ...
      self.backButton.setAttribute('class', 'back btn btn-primary');
      ...
      self.nextButton.setAttribute('class', 'next btn btn-primary');
    ...
    steps          : {
      type        : {
        draw    : function () {
          ...
          elements.heading = document.createElement('h3');
      ...
      descriptions: {
        draw    : function () {
          ...
          shortDescExplanation.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Why?') + '</h3>' + OpenLayers.Lang.translate('A short description is shown when the mouse cursor is held over the embedded map.');
          ...
          longDescExplanation.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Why?') + '</h3>' + OpenLayers.Lang.translate('A long description is used when there is an error, or when the user is unable to see the map.');
    ...
    adjustFeatureOuterBounds: function () {
      var self = this, polygon, outerBounds;
    ...
    confirmAddedMarker           : function (event, value) {
      ...
      this.data.markers.push({
        x      : this.addedMarker.geometry.x,
        y      : this.addedMarker.geometry.y,
        label  : text,
        feature: this.addedMarker
      });
    ...
    addMarkerToMarkerList        : function (markerData) {
      ...
      removeMarkerElement.innerHTML = OpenLayers.Util.hideFromOldIE(this.crossSVG) + '<span>' + OpenLayers.Lang.translate('Delete marker') + '</span>';
    ...
    embedMarkerPointSelectHandler: function (evt, ignorFieldFocus) {
      var feature,
        popup,
        label,
        input,
      ...
      feature = NK.functions.addLabeledMarker([evt.lonLat.lon, evt.lonLat.lat, ''], number, false);
      ...
      feature.okButton.setAttribute('class',  'embed-add-marker btn btn-default');
      ...
      feature.cancelButton.setAttribute('class', 'embed-cancel-marker btn btn-default');
      ...
      // BugFix:
      // Bruk av kartutsnitt i punktmenyen - tastatur forsvinner ved navngiving av markør/punkt.
      if (!ignorFieldFocus) feature.input.focus();
...
OpenLayers.Control.EmbedMarkerPointSelector = OpenLayers.Class(OpenLayers.Control, {
  ...
  redraw    : function (evt) {
    var c = document.getElementsByClassName('instructions')[0];
    c.innerHTML = '';
