...
OpenLayers.Control.Embed = 
    ...
    mock: function () {
        ...
        this.data.zoom = 12;
        this.data.markers = [
            {x: 378604, y: 7226208, label: 'Test av embed. Dette er beskrivelse av markør 1.'},
    ...
    steps: {
        type: {
            ...
            remove: function () {
                ...
                OpenLayers.Element.removeClass(panel, 'type');

                OpenLayers.Event.stopObservingElement(elements.staticButton);
                elements.staticButton.parentNode.removeChild(elements.staticButton);
                elements.staticButton = null;
                delete elements.staticButton;

                OpenLayers.Event.stopObservingElement(elements.dynamicButton);
                elements.dynamicButton.parentNode.removeChild(elements.dynamicButton);
                elements.dynamicButton = null;
                delete elements.dynamicButton;
            }
        },
        terms: {
            draw: function () {
        ...
        area: {
            draw: function () {
                ...
                this.areaHandler = new OpenLayers.Handler.ResizableBox( this,
                            {done: this.areaSelected}/*, {keyMask: this.keyMask}*/ );
        ...
        preview: {
            draw: function () {
                ...
                elements.map = document.createElement('iframe');
                elements.map.setAttribute('src', url);
                elements.map.setAttribute('width', this.data.width + 'px');
                elements.map.setAttribute('height', this.data.height + 'px');
    ...
    areaSelected: function (position) {
        var center = this.map.getLonLatFromPixel(position.getCenterPixel());
        this.data.width = Math.abs(position.getWidth());
        this.data.height = Math.abs(position.getHeight());
