...
OpenLayers.Control.Embed = 
    ...
    setMap: function (map, tracking) {
        ...
        var embedMarkerAdder;

        embedMarkerAdder = new OpenLayers.Control.EmbedMarkerPointSelector();
        this.map.addControls([embedMarkerAdder]);
        this.markerAdder = embedMarkerAdder;
    ...
    updateStepProgressPanel: function () {
        ...
        OpenLayers.Element.removeClass(this.stepProgressPanel, 'descriptions-active');
        OpenLayers.Element.removeClass(this.stepProgressPanel, 'preview-active');
        OpenLayers.Element.removeClass(this.stepProgressPanel, 'output-active');
    ...
    steps: {
        type: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'type');

                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Select type');
                ...
                OpenLayers.Event.observe(elements.staticButton, 'click', function (evt) {

                    that.selectType(that.stepSpecificElements.staticButton.value);
                });
            ...
            remove: function () {
                OpenLayers.Element.removeClass(this.stepSpecificPanel, 'type');
        ...
        terms: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'terms');
                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Terms');
                panel.appendChild(elements.heading);

                elements.p1 = document.createElement('p');
                elements.p1.innerHTML = OpenLayers.Lang.translate('You can use the maps freely for any internet based purposes. You are not allowed to combine multiple maps to form a bigger, continous map.');
                panel.appendChild(elements.p1);

                elements.p2 = document.createElement('p');
                elements.p2.innerHTML = OpenLayers.Lang.translate('Svalbard terms');
                panel.appendChild(elements.p2);

                elements.p3 = document.createElement('p');
                elements.p3.innerHTML = OpenLayers.Lang.translate('Content providers: Statens kartverk, Geovekst, municipalities and Norge digitalt partners');
                panel.appendChild(elements.p3);

            },
            remove: function () {
                var panel = this.stepSpecificPanel;
                OpenLayers.Element.removeClass(panel, 'terms');
                this.removeStepSpecificElements();
            }
        },
        area: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'area');
                ...
                */
                ...
                elements.drawRadio = document.createElement('input');
                elements.drawRadio.setAttribute('id', 'defineAreaModeDraw');
                elements.drawRadio.setAttribute('type', 'radio');
                elements.drawRadio.setAttribute('name', 'mode');
                elements.drawRadio.setAttribute('checked', 'checked'); // default mode

                elements.drawLabel = document.createElement('label');
                elements.drawLabel.setAttribute('for', 'defineAreaModeDraw');
                elements.drawLabel.innerHTML = OpenLayers.Lang.translate('Draw area');

                elements.fixedRadio = document.createElement('input');
                elements.fixedRadio.setAttribute('id', 'defineAreaModeFixed');
                elements.fixedRadio.setAttribute('type', 'radio');
                elements.fixedRadio.setAttribute('name', 'mode');
                elements.fixedRadio.setAttribute('disabled', 'disabled');

                elements.fixedLabel = document.createElement('label');
                elements.fixedLabel.setAttribute('for', 'defineAreaModeFixed');
                elements.fixedLabel.innerHTML = OpenLayers.Lang.translate('Fixed size area');

                elements.fixedInputsContainer = document.createElement('div');
                elements.fixedInputsContainer.setAttribute('class', 'fixed-inputs-container');

                elements.fixedWidthInput = document.createElement('input');
                elements.fixedWidthInput.setAttribute('type', 'text');
                elements.fixedWidthInput.setAttribute('id', 'fixedAreaModeWidthInput');
                elements.fixedWidthInput.setAttribute('placeholder', OpenLayers.Lang.translate('Width (px)'));

                elements.fixedInputsX = document.createElement('span');
                elements.fixedInputsX.setAttribute('class', 'x');
                elements.fixedInputsX.innerHTML = 'x';

                elements.fixedHeightInput = document.createElement('input');
                elements.fixedHeightInput.setAttribute('type', 'text');
                elements.fixedHeightInput.setAttribute('id', 'fixedAreaModeHeightInput');
                elements.fixedHeightInput.setAttribute('placeholder', OpenLayers.Lang.translate('Height (px)'));

                elements.modeOptionsContainer.appendChild(elements.drawRadio);
                elements.modeOptionsContainer.appendChild(elements.drawLabel);
                elements.modeOptionsContainer.appendChild(elements.fixedRadio);
                elements.modeOptionsContainer.appendChild(elements.fixedLabel);

                elements.fixedInputsContainer.appendChild(elements.fixedWidthInput);
                elements.fixedInputsContainer.appendChild(elements.fixedInputsX);
                elements.fixedInputsContainer.appendChild(elements.fixedHeightInput);

                OpenLayers.Event.observe(elements.fixedRadio, 'click', function(){
                    OpenLayers.Element.addClass(elements.fixedInputsContainer, 'on');
                });

                OpenLayers.Event.observe(elements.drawRadio, 'click', function(){
                    OpenLayers.Element.removeClass(elements.fixedInputsContainer, 'on');
                });

                OpenLayers.Event.observe(elements.fixedWidthInput, 'keyup', function(){
                    var val = parseInt(this.value);
                    if (typeof val === 'number') {
                        console.log('Width: ' + val + 'px');
                    } else {
                        console.log('Width input not valid number.');
                    }
                });

                OpenLayers.Event.observe(elements.fixedHeightInput, 'keyup', function(){
                    var val = parseInt(this.value);
                    if (typeof val === 'number') {
                        console.log('Height: ' + val + 'px');
                    } else {
                        console.log('Height input not valid number.');
                    }
                });
                ...
                if (this.data.type === 'DYNAMIC') {
                    ...
                    elements.includeTools.innerHTML = OpenLayers.Lang.translate('Include map tools');
                    ...
                    elements.label.innerHTML = OpenLayers.Lang.translate('Include map tools');
                    elements.modeOptionsContainer.appendChild(elements.label);

                    OpenLayers.Event.observe(elements.includeTools, 'change', function (evt) {
                        var include = !!this.checked;
                        that.setIncludeTools(include);
                    });
                }
                ...
                /*
                this.areaHandler = new OpenLayers.Handler.ResizableBox(this, {done:this.areaSelected});
            ...
            remove: function () {
                ...
                OpenLayers.Element.removeClass(panel, 'area');
                this.removeStepSpecificElements();
            }
        },
        markers: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'markers');

                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Include markers');
                panel.appendChild(elements.heading);
                ...
                OpenLayers.Element.addClass(this.map.viewPortDiv, 'nkAddMarker');
            ...
            remove: function () {
                ...
                OpenLayers.Element.removeClass(panel, 'markers');
                if (this.addedMarker) {
                ...
                if (elements.markerRemoveLinks) {
                    while (elements.markerRemoveLinks.length > 0) {
                        ...
                        OpenLayers.Event.stopObservingElement(r);
                ...
                OpenLayers.Element.removeClass(this.map.viewPortDiv, 'nkAddMarker');
            }
        },
        descriptions: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'descriptions');
                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Map description');
                panel.appendChild(elements.heading);

                elements.recommendation = document.createElement('p');
                elements.recommendation.setAttribute('class', 'recommendation');
                elements.recommendation.innerHTML = OpenLayers.Lang.translate('Her kommer en tekst som sterkt anbefaler å legge inn en beskrivelse.');
                ...
                elements.shortDescLabel.innerHTML = OpenLayers.Lang.translate('Short description:');
                panel.appendChild(elements.shortDescLabel);

                elements.shortDescExample = document.createElement('span');
                elements.shortDescExample.innerHTML = OpenLayers.Lang.translate('I.ex.') + '<em>' + OpenLayers.Lang.translate('"Stores in Hordaland"') + '</em>';
                ...
                elements.shortDescWhy.innerHTML = OpenLayers.Lang.translate('Why?');

                shortDescExplanation = document.createElement('div');
                shortDescExplanation.setAttribute('id', 'short-desc-explanation');
                shortDescExplanation.innerHTML = '<h2 class="h">' + OpenLayers.Lang.translate('Why?') + '</h2>' + OpenLayers.Lang.translate('A short description is shown when the mouse cursor is held over the embedded map.');
                ...
                elements.shortDescExplanationClose.innerHTML = OpenLayers.Lang.translate('close');
                ...
                OpenLayers.Event.observe(elements.shortDescWhy, 'click', function (evt) {
                    evt.stopPropagation();
                    evt.preventDefault();
                    OpenLayers.Element.addClass(evt.target.parentNode, 'active');
                    return false;
                }, true);

                OpenLayers.Event.observe(elements.shortDescExplanationClose, 'click', function (evt) {
                    evt.stopPropagation();
                    evt.preventDefault();
                    OpenLayers.Element.removeClass(evt.target.parentNode.parentNode, 'active');
                    return false;
                }, true);
                ...
                OpenLayers.Event.observe(elements.shortDescInput, 'change', function (evt) {
                    that.setShortDesc(this.value);
                });                
                ...
                elements.longDescLabel.innerHTML = OpenLayers.Lang.translate('Long description:');
                ...
                elements.longDescWhy.innerHTML = OpenLayers.Lang.translate('Why?');

                longDescExplanation = document.createElement('div');
                longDescExplanation.setAttribute('id', 'long-desc-explanation');
                longDescExplanation.innerHTML = '<h2 class="h">' + OpenLayers.Lang.translate('Why?') + '</h2>' + OpenLayers.Lang.translate('A long description is used when there is an error, or when the user is unable to see the map.');
                ...
                elements.longDescExplanationClose.innerHTML = OpenLayers.Lang.translate('close');
                ...
                OpenLayers.Event.observe(elements.longDescWhy, 'click', function (evt) {
                    evt.stopPropagation();
                    evt.preventDefault();
                    OpenLayers.Element.addClass(evt.target.parentNode, 'active');
                    return false;
                ...
                OpenLayers.Event.observe(elements.longDescExplanationClose, 'click', function (evt) {
                    evt.stopPropagation();
                    evt.preventDefault();
                    OpenLayers.Element.removeClass(evt.target.parentNode.parentNode, 'active');
                    return false;
                ...
                OpenLayers.Event.observe(elements.longDescInput, 'change', function (evt) {
                    that.setLongDesc(this.value);
                });
            ...
            */
            remove: function () {
                ...
                OpenLayers.Element.removeClass(panel, 'descriptions');

                OpenLayers.Event.stopObservingElement(elements.longDescExplanationClose);
                ...
                OpenLayers.Event.stopObservingElement(elements.shortDescWhy);
                elements.shortDescWhy.parentNode.removeChild(elements.shortDescWhy);
                elements.shortDescWhy = null;
                delete elements.shortDescWhy;

                OpenLayers.Event.stopObservingElement(elements.shortDescExplanationClose);
                ...
                OpenLayers.Event.stopObservingElement(elements.longDescWhy);
        ...
        preview: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'preview');
                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Generate map');
                panel.appendChild(elements.heading);

                elements.shortDesc = document.createElement('section');
                elements.shortDesc.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Short description:') + '</h3><span>' + this.data.shortDesc + '</span>';
                panel.appendChild(elements.shortDesc);

                elements.longDesc = document.createElement('section');
                elements.longDesc.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Long description:') + '</h3><span>' + this.data.longDesc + '</span>';
                panel.appendChild(elements.longDesc);


                elements.mapContainer = document.createElement('section');
                OpenLayers.Element.addClass(elements.mapContainer, 'map-container');
                ...
                this.nextButton.innerHTML = OpenLayers.Lang.translate('Generate code');

                var widget = document.getElementById('PMwidget');
                if (widget && $(widget).offset().top < 0) {
                    OpenLayers.Element.addClass(panel, 'limit-height');
                }
            },
            remove: function () {
                var panel = this.stepSpecificPanel;
                OpenLayers.Element.removeClass(panel, 'preview');
                this.removeStepSpecificElements();
        ...
        output: {
            draw: function () {
                ...
                OpenLayers.Element.addClass(panel, 'output');
                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Generated code');
                panel.appendChild(elements.heading);

                elements.embedCodeOutput = document.createElement('div');
                elements.embedCodeOutput.setAttribute('class', 'output-code');
                elements.embedCodeOutput.innerHTML = OpenLayers.Util.htmlEncode(this.getIframe().outerHTML);
                panel.appendChild(elements.embedCodeOutput);
                this.nextButton.innerHTML = OpenLayers.Lang.translate('Quit');
            },
            remove: function () {
                var panel = this.stepSpecificPanel;
                OpenLayers.Element.removeClass(panel, 'output');
                this.removeStepSpecificElements();
                this.nextButton.innerHTML = OpenLayers.Lang.translate('Next');
            }
        }
    ...
    disableMapMovement: function(){
        /*
        console.log('disableMapMovement');
        this.map.controls[15].deactivate();
        this.map.controls[20].disable();
        this.map.controls[25].disable();
        */
    },

    enableMapMovement: function(){
        /*
        this.map.controls[15].activate();
        this.map.controls[20].enable();
        this.map.controls[25].enable();
        */
    },

    translateFeatureToMask: function(){

        var self = this, b, tlp, brp, pixelBounds, width, height, 
            nodes = self.attr['nodes'], styles, viewport;

        if (!self.areaSelectedBounds || !self.featureMask) return;

        self.enableMaskControls();

        b = self.areaSelectedBounds.bounds;
        viewport = self.map.getSize();
        ...
        width = Math.abs(pixelBounds.getWidth());
        height = Math.abs(pixelBounds.getHeight());

        styles = 'top:0px;bottom:0px;left:0px;width:'+tlp.x+'px;height:'+viewport.h+'px;';
        nodes['left'].setAttribute('style', styles);

        styles = 'top:0px;height:'+tlp.y+'px;left:'+tlp.x+'px;width:'+width+'px;';
        nodes['top'].setAttribute('style', styles);

        styles = 'top:0px;bottom:0px;left:'+(tlp.x + width)+'px;right:0px;height:'+viewport.h+'px;';
        nodes['right'].setAttribute('style', styles);

        styles = 'top:'+brp.y+'px;bottom:0px;left:'+tlp.x+'px;width:'+width+'px;';
        nodes['bottom'].setAttribute('style', styles);

    },

    translateMaskToFeature: function(){

        if (!this.attr['nodes'] || !this.div || !OpenLayers.Element.hasClass(this.div, 'active')) return;

        var self = this, size, outerBounds, innerBounds, polygon, 
            el, tl, tr, bl, br, p1, p2, p3, p4, pts = [];
        ...
        outerBounds = new OpenLayers.Geometry.LinearRing(pts);

        // Define inner bounds of mask
        pts = [];

        tl = self.map.getLonLatFromPixel({
            x : parseInt($(self.attr['nodes'].top).css('left')),
            y : parseInt($(self.attr['nodes'].top).css('height'))
        });
        tr = self.map.getLonLatFromPixel({
            x : parseInt($(self.attr['nodes'].top).css('left')) + 
                    parseInt($(self.attr['nodes'].top).css('width')),
            y : parseInt($(self.attr['nodes'].top).css('height'))
        });
        bl = self.map.getLonLatFromPixel({
            x : parseInt($(self.attr['nodes'].bottom).css('left')),
            y : parseInt($(self.attr['nodes'].bottom).css('top'))
        });
        br = self.map.getLonLatFromPixel({
            x : parseInt($(self.attr['nodes'].bottom).css('left')) + 
                    parseInt($(self.attr['nodes'].bottom).css('width')),
            y : parseInt($(self.attr['nodes'].bottom).css('top'))
        });
        ...
        innerBounds = new OpenLayers.Geometry.LinearRing(pts);
        self.areaSelectedBounds = innerBounds;
    ...
    removeAddedMarkerEditContent: function() {
        var popup;
        if (this.addedMarker && this.addedMarker.popup) {
            popup = this.addedMarker.popup;
    ...
    removeMarkerFeature: function (marker) {
        if (marker && marker.popup) {
            marker.popup.feature = null;
            if (this.addedMarker && marker === this.addedMarker) {
                this.removeAddedMarkerEditContent();
            }
    ...
    confirmAddedMarker: function (event, value) {
        var text = value || this.addedMarker.input.value;
        this.removeAddedMarkerEditContent();

        if (text) {
            text = text.replace(/^[ \t]*/,'').replace(/[ \t]*$/,'');
            text = OpenLayers.Util.htmlEncode(text);
        } 
        if (text) {
            this.addedMarker.popup.setContentHTML(text);
            this.addedMarker.popup.updateSize();
        } else {
            text = null;
        }
        this.data.markers = this.data.markers || [];
        this.data.markers.push({x: this.addedMarker.geometry.x, y: this.addedMarker.geometry.y, label: text, feature: this.addedMarker});
        this.addedMarker.attributes.nr = this.data.markers.length;
        this.addedMarker.layer.drawFeature(this.addedMarker, 'default');
        this.addMarkerToMarkerList(this.data.markers[this.data.markers.length - 1]);
        this.addedMarker = null;

    },
    addMarkerToMarkerList: function (markerData) {
        var panel = this.stepSpecificPanel,
            elements = this.stepSpecificElements,
            that = this,
            markerListItem,
        ...
        if (!elements.markerList) {
            elements.markerList = document.createElement('ol');
            elements.markerList.setAttribute('class', 'markers-list');
            elements.markerRemoveLinks = [];
            panel.appendChild(elements.markerList);        
        }

        id = OpenLayers.Util.createUniqueID('');
        markerListItem = document.createElement('li');
        markerListItem.setAttribute('id', 'marker-' + id);
        markerListItem.innerHTML = '<span>' + markerData.label + '</span>';
        removeMarkerElement = document.createElement('a');
        removeMarkerElement.innerHTML = OpenLayers.Lang.translate('Delete marker');
        removeMarkerElement.setAttribute('href', '#remove-marker-' + id);
        removeMarkerElement.setAttribute('class', 'remove-marker');
        removeMarkerElement.setAttribute('id', 'remove-marker-' + id);
        markerListItem.appendChild(removeMarkerElement);
        elements.markerRemoveLinks.push(removeMarkerElement); // keep reference 
        elements.markerList.appendChild(markerListItem);
        markerData.id = id;

        OpenLayers.Event.observe(removeMarkerElement, 'click', function (evt) {
            var hrefAttr, 
                markerId;
            ...
            hrefAttr = evt.currentTarget.getAttribute('href');
            markerId = parseInt(hrefAttr.replace('#remove-marker-', ''));
            that.removeMarker(markerId);
        });
    },
    embedMarkerPointSelectHandler: function (evt) {
        var feature, 
            popup, 
            label, 
            input, 
        ...
        if (this.div && OpenLayers.Element.hasClass(this.div, 'active')) {
        	var bool =  this.areaSelectedBounds && 
                        this.areaSelectedBounds.containsPoint(new OpenLayers.Geometry.Point(evt.lonLat.lon, evt.lonLat.lat));
        	if ( !bool ) return;
        }
	
        feature = NK.functions.addLabeledMarker([evt.lonLat.lon, evt.lonLat.lat]);
        feature.popup.setContentHTML('');
	    
        label = document.createElement('label'); 
        label.setAttribute('for', 'added-marker-description');
        label.innerHTML = OpenLayers.Lang.translate('Comment');
        feature.popup.contentDiv.appendChild(label);

        feature.input = document.createElement('input');
        feature.input.setAttribute('type', 'text');
        feature.input.setAttribute('id', 'added-marker-description');
        feature.popup.contentDiv.appendChild(feature.input);

        feature.okButton = document.createElement('button');
        feature.okButton.setAttribute('class', 'embed-add-marker');
        feature.okButton.innerHTML = OpenLayers.Lang.translate('Add');
        OpenLayers.Event.observe(feature.okButton, 'click', function (evt) {
            that.confirmAddedMarker(); 
            return false;
        });
        feature.popup.contentDiv.appendChild(feature.okButton);

        feature.cancelButton = document.createElement('button');
        feature.cancelButton.setAttribute('class', 'embed-cancel-marker');
        feature.cancelButton.innerHTML = OpenLayers.Lang.translate('Skip');
        OpenLayers.Event.observe(feature.cancelButton, 'click', function(evt) {
            that.removeMarkerFeature(that.addedMarker); 
            return false;
        });
        feature.popup.contentDiv.appendChild(feature.cancelButton);

        OpenLayers.Element.addClass(feature.popup.contentDiv, 'edit-popup-content');
        feature.popup.updateSize();

        feature.input.focus();
        this.addedMarker = feature;
    },
    getIframe: function ( onPreview, panel ) {
        ...
        iframe.setAttribute('title', this.data.shortDesc);
        iframe.setAttribute('longdesc', this.data.longDesc);
        return iframe;
    },
    getURL: function () {
        var hash,
    ...
    areaSelected: function (feature) {
        ...
        center = feature.geometry.bounds.getCenterLonLat();
        b = feature.geometry.bounds;
        this.selectedAreaBounds = b;
        ...
        this.data.width = Math.abs(pixelBounds.getWidth());
        this.data.height = Math.abs(pixelBounds.getHeight());
        this.data.centerX = parseInt(center.lon);
        this.data.centerY = parseInt(center.lat);

        feature.attributes.height = this.data.height;
        feature.attributes.width = this.data.width;
        this.boxLayer.drawFeature(this.selectedArea, 'default');
        var me = this.map.getMaxExtent();
        var boxLayer = this.boxLayer;

        var addMask = function (left, top, right, bottom) {
            var boundsTop = new OpenLayers.Bounds();
            boundsTop.extend(new OpenLayers.LonLat(left, top));
            boundsTop.extend(new OpenLayers.LonLat(right, bottom));
       
            var maskTop = new OpenLayers.Feature.Vector(
                boundsTop.toGeometry(), 
                null,
                {
                    fillOpacity: 0.2,
                    strokeWidth: 0,
            ...
            boxLayer.addFeatures([maskTop]);
        };

        addMask(me.left, me.top, me.right, feature.geometry.bounds.top); // top mask
        addMask(me.left, feature.geometry.bounds.top, feature.geometry.bounds.left, feature.geometry.bounds.bottom); // left mask
        addMask(feature.geometry.bounds.right, feature.geometry.bounds.top, me.right, feature.geometry.bounds.bottom); // right mask
        addMask(me.left, feature.geometry.bounds.bottom, me.right, me.bottom); // bottom mask
	...
	*/
    ...
    describeSquareSize : function() {
        ...
        console.log('describeSquareSize');
    ...
    moveDragArea : function( e ) {
        ...
        console.log('moveDragArea');

	var self = this, attr = self.attr, nodes = attr['nodes'] || {};
	if ( ! nodes['base'] || ! attr['downEvent'] ) return;

	var dTarget = attr['downEvent'].target;
	if ( OpenLayers.Element.hasClass(dTarget,'tool') ) {
	    OpenLayers.Element.removeClass(nodes['base'],'adjust');
	    var isTop  = OpenLayers.Element.hasClass(dTarget,'top');
	    var isLeft = OpenLayers.Element.hasClass(dTarget,'left');

	    if ( isTop && isLeft ) {         // top - left
		attr['points'][0] = [e.clientX, e.clientY];
	    }
	    else if ( isTop && ! isLeft )  { // top - right
		attr['points'][0][1] = e.clientY;
		attr['points'][1][0] = e.clientX;		
	    }
	    else if ( isLeft ) {             // bottom - left
		attr['points'][0][0] = e.clientX;
		attr['points'][1][1] = e.clientY;		
	    }
	    else {                           // bottom - right
		attr['points'][1] = [e.clientX, e.clientY]; 
	    }
	    
	    var allow = attr['points'][0][0] < attr['points'][1][0] &&
		attr['points'][0][1] < attr['points'][1][1];
	    
	    if ( allow ) self.setDragAreaShadow( attr['points'] );
	}
	else if ( nodes['square'] && self.activeStep == 'area' ) {
	    var x = [attr['downEvent'].clientX, e.clientX, e.clientX];
	    var y = [attr['downEvent'].clientY, e.clientY, e.clientY];
	    
	    if ( x[0] > x[1] ) { x[1] = x[0], x[0] = x[2]; }
	    if ( y[0] > y[1] ) { y[1] = y[0], y[0] = y[2]; }
	    
	    var s = 'left:'+x[0]+'px;top:'+y[0]+'px;'+
		'width:'+(x[1]-x[0])+'px;height:'+(y[1]-y[0])+'px;';
	...
	if ( nodes['base'] && OpenLayers.Element.hasClass(nodes['base'],'adjust') ) {
		...
		if ( self.activeStep == 'area' ) self.resetDragArea();
	    }
	}
	attr['downEvent'] = e;	
    },
    ...
    insertDragArea : function( e ) {
        ...
        console.log('insertDragArea');

	var self = this, attr = self.attr, wrapper = document.body;
	...
	for (var i=0; i<attr['names'].length; i++ ) {
	    var name   = attr['names'][i]; 
	    var target = document.getElementById( attr['type']+'OF'+name );
	    if ( target ) target.parentNode.removeChild( target );
	    nodes[name] = null;
	}
	attr['downEvent'] = null, attr['points'] = null; 
	attr['events'] = null, attr['nodes'] = null;
	...
	var self = this, attr = self.attr, nodes = attr['nodes'] || {};
	for (var i=0; i<attr['names'].length; i++ ) {
	    var name = attr['names'][i], target = document.getElementById( 
		attr['type']+'OF'+name 
	    ) || nodes[name];
	    if ( ! target ) continue;
	...
	attr['downEvent'] = null, attr['points'] = null, attr['events'] = null;
    },
    
    CLASS_NAME: "OpenLayers.Control.Embed"
}); // OpenLayers.Control.Transformations

 OpenLayers.Control.EmbedMarkerPointSelector = OpenLayers.Class(OpenLayers.Control, {
    initialize: function (options) {
        OpenLayers.Control.prototype.initialize.apply(this, [options]);
    },    
    redraw: function(evt) {
        ...
        this.map.events.triggerEvent("embedMarkerPointSelected", {'lonLat': lonLat});
    },
    /**
     * APIMethod: activate
     */
    ...
    CLASS_NAME: "OpenLayers.Control.EmbedMarkerPointSelector"
});
