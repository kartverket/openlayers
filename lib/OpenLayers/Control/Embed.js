...
OpenLayers.Control.Embed = 
    ...
    initialize: function(options) {
        ...
        OpenLayers.Util.extend(OpenLayers.Lang.nb, {
            ...
            'Content providers: Statens kartverk, Geovekst, municipalities and Norge digitalt partners': 'Innholdsleverandører: Statens kartverk, Geovekst, kommuner og Norge digitalt-parter.',
            'Previous': 'Forrige',
            'Next': 'Neste',
            'Show selected map area': 'Vis kartutsnitt',
            'Choose area': 'Velg utsnitt',
            'Include map tools': 'Ta med kartverktøy',
            'Include markers': 'Sett inn markør',
            'Delete marker': 'Slett markør',
    ...
    mock: function () {
        this.data.includeTools = true;
        this.data.type = 'STATIC';
        this.data.shortDesc = 'Test av embed. Dette er kort beskrivelse';
        this.data.longDesc = 'Test av lengre beskrivelse. Hvorfor er det viktig med en lengre beskrivelse? Det vil tiden vise...';
        this.data.centerX = 378604;
        this.data.centerY = 7226208;
        this.data.width = 450;
        this.data.height = 250;
        this.data.zoom = 12;
        this.data.markers = [
            {x: 291495, y: 7037470, label: 'Test av embed. Dette er beskrivelse av markør 1.'},
            {x: 378904, y: 7226208, label: 'Test av embed. Dette er beskrivelse av markør 2.'},
            {x: 378004, y: 7226208, label: 'Test av embed. Dette er beskrivelse av markør 3.'}
        ];
    },
    ...
    draw: function () {
        ...
        OpenLayers.Event.observe(this.backButton, 'click', function (evt) {
            evt.stopPropagation();
            evt.preventDefault();
            self.previousStep();
        });

        OpenLayers.Event.observe(this.nextButton, 'click', function (evt) {
            evt.stopPropagation();
            evt.preventDefault();
            self.nextStep();
        });
    ...
    steps: {
        type: {
            draw: function () {
                ...
                elements.dynamicButton.setAttribute('value', 'DYNAMIC');
                ...
                elements.staticButton.setAttribute('value', 'STATIC');
        ...
        area: {
            draw: function () {
                ...
                if (this.data.type === 'DYNAMIC') {
                    ...
                    panel.appendChild(elements.label);
                    ...
                    panel.appendChild(elements.includeTools);
                ...
                if (!this.boxLayer) {
                    this.boxLayer = new OpenLayers.Layer.Vector('embedBoxLayer', {
                        shortid: "embed.box", 
                        styleMap: NK.styles.embed.box, 
                        visibility: true
                    });
                    this.map.addLayers([this.boxLayer]);
                    boxLayerJustAdded = true;
                }
                if (!this.boxControl) {
                    this.boxControl = new OpenLayers.Control.DrawFeature(this.boxLayer,
                        OpenLayers.Class(OpenLayers.Handler.RegularPolygon, {
                            move: function(evt) {
                                /* 
                                 * modified version of RegularPolygon.move 
                                 * updates pixel measures in the attributes 'width' and 'height'
                                 */
                                ...
                                this.setFeatureMeasures(this.feature);
                            },
                            down: function(evt) {
                                /*
                                 * modified version of RegularPolygon.down
                                 * updates pixel measures in the attributes 'width' and 'height' before drawing
                                 */
                            ...
                            setFeatureMeasures: function (feature) {
                                var bounds,
                                    topLeft,
                                    bottomRight,
                                    pixelBounds;
                                ...
                                topLeft = this.map.getPixelFromLonLat(new OpenLayers.LonLat(bounds.left, bounds.top));
                                bottomRight = this.map.getPixelFromLonLat(new OpenLayers.LonLat(bounds.right, bounds.bottom));
                                ...
                                feature.attributes.width = Math.abs(pixelBounds.getWidth());
                                feature.attributes.height = Math.abs(pixelBounds.getHeight());
                    ...
                    this.boxControl.events.register('featureadded', this, function (evt) {
                        this.selectedArea = evt.feature;
                        this.areaSelected(this.selectedArea);
                    });
                    this.map.addControl(this.boxControl);
                } else if (boxLayerJustAdded) {
                    this.boxControl.layer = this.boxLayer;
                }
                this.boxControl.activate();
                this.map.events.register('zoomend', this, function(evt){ 
    ...
    confirmAddedMarker: function (event) {
        var text;

        text = this.addedMarker.input.value;
        this.removeAddedMarkerEditContent();
    ...
    embedMarkerPointSelectHandler: function (evt) {
        ...
        if (this.selectedAreaBounds && this.selectedAreaBounds.containsLonLat(evt.lonLat)) {
    ...
    getIframe: function () {
        ...
        iframe.setAttribute('width', this.data.width + 'px');
        iframe.setAttribute('height', this.data.height + 'px');
    ...
    getURL: function () {
        ...
        hash = window.location.hash.replace(/#([0-9]+\/){3}/, '#' + this.map.zoom + '/' + this.data.centerX + '/' + this.data.centerY + '/');
        ...
        url = window.location.origin + window.location.pathname + extraPath + hash;
    ...
    hideControls: function () { 
        ...
        for (d in this.data) {
            if (this.data.hasOwnProperty(d)) {
                if (d === 'markers') {
                    for (i = 0, j = this.data[d].length; i < j; i += 1) {
                        if (this.data[d][i].feature) {
                            this.removeMarkerFeature(this.data[d][i].feature);
                            this.data[d][i].feature = null;
                            delete this.data[d][i].feature;
                        }
                    }
                }
                this.data[d] = null;
        ...
        this.steps[this.activeStep].remove.apply(this);
        this.activeStep = null;
        if (this.areaElement) {
            OpenLayers.Event.stopObservingElement(this.areaElement);
            this.areaElement.parentNode.removeChild(this.areaElement);
            this.areaElement = null;            
        }
        if (this.boxControl) {
            this.boxControl.deactivate();
            this.map.removeControl(this.boxControl);
            this.boxControl = null;
            delete this.boxControl;
        }
        if (this.boxLayer) {
