...
OpenLayers.Control.Embed = 
    ...
    steps: {
		...
		var attr = {'lon':252005, 'lat':6658205};
                    ...
                    this.boxControl = new OpenLayers.Control.DrawFeature(this.boxLayer,
                        OpenLayers.Class(OpenLayers.Handler.RegularPolygon, {
                            ...
                            setFeatureMeasures: function (feature) {
                                ...
                                topLeft = this.map.getPixelFromLonLat(new OpenLayers.LonLat(
				    bounds.left, bounds.top
				));
                                bottomRight = this.map.getPixelFromLonLat(new OpenLayers.LonLat(
        ...
        preview: {
            draw: function () {
                ...
                elements.map = this.getIframe( true, panel );
    ...
    embedMarkerPointSelectHandler: function (evt, force ) {
        var feature, 
            popup, 
            label, 
            input, 
        ...
        this.removeMarkerFeature(this.addedMarker);
	var bool = force || this.selectedAreaBounds && this.selectedAreaBounds.containsLonLat(evt.lonLat);
	if ( ! bool ) return;
	...
	var self  = this, size = [self.data.width,self.data.height];
	var width = '', heigth = '', cnt  = panel || self.cnt;
	if ( onPreview ) {
	    var space = [cnt.clientWidth, 200]; //[cnt.clientWidth, cnt.clientHeight];
	    var left  = parseInt( OpenLayers.Util.getStyle(cnt,'padding-left') );
	    var right = parseInt( OpenLayers.Util.getStyle(cnt,'padding-right') );
	    ...
	    width  = size[0] > space[0] ? '100%' : (size[0]+'px');
	    height = size[1] > space[1] ? (space[1]+'px') : (size[1]+'px');  
	}
	else { width = size[0]+'px', height = size[1]+'px'; }
	
        var iframe = document.createElement('iframe');
        //iframe.setAttribute('src', 'http://www.vg.no');
        iframe.setAttribute('src', self.getURL());
	iframe.setAttribute('scrolling', 'yes');
        iframe.setAttribute('width', width);
        iframe.setAttribute('height', height);
        iframe.setAttribute('title', self.data.shortDesc);
        iframe.setAttribute('longdesc', self.data.longDesc);
        return iframe;
    },
    getURL: function () {
        var hash,
