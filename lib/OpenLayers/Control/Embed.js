...
OpenLayers.Control.Embed = 
    ...
    removeMarker: function (id){
        var markerElement, linkElement, elements, i, j, e;
        ...
        for (i = 0, j = this.data.markers.length; i < j; i += 1) {
            if (parseInt(this.data.markers[i].id) === parseInt(id)) {
    ...
    steps: {
        ...
        markers: {
            draw: function () {
                ...
                // TODO: fjerne alle markører fra kartet før de legges til på nytt
                ...
                if (this.data.markers && this.data.markers.length > 0) {
                    ...
                    for (i = 0, j = this.data.markers.length; i < j; i += 1) {
                        ...
                        markerListItem.innerHTML = '<span>' + this.data.markers[i].label + '</span>';
        ...
        output: {
            draw: function () {
                ...
                elements.embedCodeOutput.innerHTML = safeTagsReplace(this.getIframe().outerHTML);
    ...
    removeAddedMarker: function () {
        if (this.addedMarker) {
            this.addedMarker.popup.feature = null;
            this.map.removePopup(this.addedMarker.popup);
            if (this.addedMarker.cancelButton) {
                OpenLayers.Event.stopObservingElement(this.addedMarker.cancelButton);
            }
            if (this.addedMarker.okButton) {
                OpenLayers.Event.stopObservingElement(this.addedMarker.okButton);
            }
            this.addedMarker.destroy();
            this.addedMarker = null;
        }
    },
    embedMarkerPointSelectHandler: function (evt) {
        ...
        if (this.selectedAreaBounds && this.selectedAreaBounds.containsLonLat(evt.lonLat)) {
            this.addedMarker = NK.functions.addLabeledMarker([evt.lonLat.lon, evt.lonLat.lat]);
            this.addedMarker.popup.setContentHTML('');

            label = document.createElement('label');
            label.setAttribute('for', 'marker-description');
            label.innerHTML = 'beskrivelse';
            this.addedMarker.popup.contentDiv.appendChild(label);

            input = document.createElement('input');
            input.setAttribute('type', 'text');
            input.setAttribute('id', 'marker-description');
            this.addedMarker.popup.contentDiv.appendChild(input);

            this.addedMarker.cancelButton = document.createElement('button');
            this.addedMarker.cancelButton.innerHTML = 'avbryt';
            OpenLayers.Event.observe(this.addedMarker.cancelButton, 'click', function(evt) {
                that.removeAddedMarker(); 
                return false;
            });
            this.addedMarker.popup.contentDiv.appendChild(this.addedMarker.cancelButton);

            this.addedMarker.okButton = document.createElement('button');
            this.addedMarker.okButton.innerHTML = 'fortsett';
            this.addedMarker.popup.contentDiv.appendChild(this.addedMarker.okButton);

            OpenLayers.Element.addClass(this.addedMarker.popup.contentDiv, 'edit-popup-content');
...
//        console.log('clicked at ' + lonLat.lon + ' ' + lonLat.lat);
    ...
    activate: function() {
        if (OpenLayers.Control.prototype.activate.apply(this, arguments)) {
            ...
            console.log('embedMarkerPointSelector active');
            return true;
        } else {
            return false;
        }
