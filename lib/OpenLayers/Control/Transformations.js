...
OpenLayers.Control.Transformations =
    ...
    initialize: function(options) {
        ...
        this.sosiCodes = [
            ...
            {'ESRI': 32631, 'EPSG': 32631, 'SOSI': 21, 'name': 'EU89, UTM-sone 31', 'viewable': true},
            {'ESRI': 32632, 'EPSG': 32632, 'SOSI': 22, 'name': 'EU89, UTM-sone 32', 'viewable': true},
            {'ESRI': 32633, 'EPSG': 32633, 'SOSI': 23, 'name': 'EU89, UTM-sone 33', 'viewable': true},
            {'ESRI': 32634, 'EPSG': 32634, 'SOSI': 24, 'name': 'EU89, UTM-sone 34', 'viewable': true},
            {'ESRI': 32635, 'EPSG': 32635, 'SOSI': 25, 'name': 'EU89, UTM-sone 35', 'viewable': true},
            {'ESRI': 32636, 'EPSG': 32636, 'SOSI': 26, 'name': 'EU89, UTM-sone 36', 'viewable': true},
            ...
            {'ESRI': null, 'EPSG': null, 'SOSI': 50, 'name': 'ED50, Geografisk', 'viewable': false},
            //{'ESRI': null, 'EPSG': null, 'SOSI': 53, 'name': 'Møre-A'},
            //{'ESRI': null, 'EPSG': null, 'SOSI': 54, 'name': 'Møre-B'},
            {'ESRI': null, 'EPSG': 4326, 'SOSI': 84, 'name': 'EU89/WGS84 Geografisk, grader', 'viewable': false} //viewable, but not necessary in selectors
    ...
    generateCoordinateSystemsList: function (id, label, selectedValue, viewableOnly) {
        var sosiCode,
            select,
            option,
            i,
    ...
    insertContent: function (coordinate) {
        ...
        inSystem = this.generateCoordinateSystemsList('transformations-input-coordinate-system', OpenLayers.Lang.translate('Select input coordinate system'), crsChoice);
        inSystem.select.setAttribute('onchange', "map.getControlsByClass('OpenLayers.Control.Transformations')[0].setCoordinateCookie(this, false)");
    ...
    specialOut: function (result) {
        if (this.eastCoordinateOutputElement) {
            this.eastCoordinateOutputElement.innerHTML = OpenLayers.Util.preciseRound(result.ost, 3);
            this.eastCoordinateOutputElement.setAttribute('title', result.ost);
        }
        if (this.northCoordinateOutputElement) {
            this.northCoordinateOutputElement.innerHTML = OpenLayers.Util.preciseRound(result.nord, 3);
    ...
    gotoCoordinate: function () {
        ...
        requestParams.ost = parseFloat(east.replace(/^\s+|\s+$/g, '').replace(/,/g, '.'));
        requestParams.nord = parseFloat(north.replace(/^\s+|\s+$/g, '').replace(/,/g, '.'));
        requestParams.sosiKoordSys = document.getElementById('transformations-input-coordinate-system').value;
