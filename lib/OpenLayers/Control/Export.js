    ...
    steps: {
        info: {
            draw: function () {
                ...
                elements.p1.innerHTML = OpenLayers.Lang.translate('This guide helps you to select a map area for print or export from the base map. The exported map will not include overlays or user data painted on the map.');
                panel.appendChild(elements.p1);

                elements.p3 = document.createElement('p');
                elements.p3.innerHTML = OpenLayers.Lang.translate('All map data is free for any use if attribution is given to "Kartverket" (License: CC-BY).');
        ...
        format: {
            draw: function () {
                ...
                elements.heading = document.createElement('h2');
                elements.heading.setAttribute('class', 'h');
                elements.heading.innerHTML = OpenLayers.Lang.translate('Choose output size and format');
                panel.appendChild(elements.heading);

                elements.recommendation = document.createElement('p');
                elements.recommendation.setAttribute('class', 'recommendation');
                elements.recommendation.innerHTML = OpenLayers.Lang.translate('From the drop down box below, select the size of the map to be exported.');
                panel.appendChild(elements.recommendation);

                elements.sizeSelector = document.createElement('span');
                var points = this.attr['points'];
	        var size = this.getSizeOfPoints( points );
                ...
                code = "<select>";
                code += "<option value='1'>Som vist ("+size[0]+"x"+size[1]+" piksler)</option>";
                code += "<option value='2'>2x ("+size[0]*2+"x"+size[1]*2+" piksler)</option>";
                if ((size[0]<1000) && (size[1]<1000)) {
                  code += "<option value='4'>4x ("+size[0]*4+"x"+size[1]*4+" piksler)</option>";
                  code += "<option value='4'>4x til trykk ("+OpenLayers.Util.preciseRound(size[0]*4*PX_TO_CM,2)+"x"+OpenLayers.Util.preciseRound(size[1]*4*PX_TO_CM,2)+"cm @ 300dpi)</option>";
                }
                if ((size[0]<500) && (size[1]<500)) {
                  code += "<option value='8'>8x ("+size[0]*8+"x"+size[1]*8+" piksler)</option>";
                  code += "<option value='8'>8x til trykk ("+OpenLayers.Util.preciseRound(size[0]*8*PX_TO_CM,2)+"x"+OpenLayers.Util.preciseRound(size[1]*8*PX_TO_CM,2)+"cm @ 300dpi)</option>";
                }
                ...
                // TODO: add a scale that fits 297 x 210mm for print (A4)
                code += "</select>&nbsp;";
                elements.sizeSelector.innerHTML = code;
                panel.appendChild(elements.sizeSelector);

                elements.formatSelector = document.createElement('span');
                code = "<select>";
                code += "<option value='image/png'>PNG</option>";
                code += "<option value='image/jpeg'>JPEG</option>";
                code += "<option value='image/geotiff'>GeoTIFF</option>";
                code += "<option value='geojpeg'>GeoJPEG</option>";
                elements.formatSelector.innerHTML = code; 
                panel.appendChild(elements.formatSelector);

                elements.output = document.createElement('div');
                panel.appendChild(elements.output);

                var updateLink = function (evt) { 
                  var format  = elements.formatSelector.children[0].value;
                  var service = "wms.topo2";
                  var layers  = "topo2_wms";
                  var pscale  = elements.sizeSelector.children[0].value;
                  ...
                  if (format == "geojpeg") {
                    ...
                    var x = bl.lon;
                    var y = tr.lat;
                    var px = (tr.lon-bl.lon)/size[0];
                    var py = (bl.lat-tr.lat)/size[1];
                    WMS_URL = "http://www.norgeskart.no/ws/geoimage.py?url=" + encodeURIComponent("http://openwms.statkart.no/skwms1/"+service+"?styles=&service=WMS&request=GetMap&SRS=EPSG:32633&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.1.1&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox)+"&px="+px+"&py="+py+"&x="+x+"&y="+y;
                  } else {
                    WMS_URL = "http://openwms.statkart.no/skwms1/"+service+"?styles=&service=WMS&request=GetMap&SRS=EPSG:32633&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.1.1&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox;
                  }
                 
                  var preview = "<img border=0 width="+prev_x+" height="+hratio+" src='http://openwms.statkart.no/skwms1/"+service+"?styles=&service=WMS&request=GetMap&SRS=EPSG:32633&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.1.1&WIDTH="+prev_x+"&HEIGHT="+hratio+"&BBOX="+bbox+"'/>";
                    elements.output.innerHTML = "<p>Forh&aring;ndsvisning:<br/><a target='_blank' href='"+WMS_URL+"'>"+preview+"<br/>Klikk her for &aring; &aring;pne/laste ned bildet.</a></p>";
                }
                updateLink(null);
              
                OpenLayers.Event.observe(elements.sizeSelector, 'change', updateLink); 
                OpenLayers.Event.observe(elements.formatSelector, 'change', updateLink); 
