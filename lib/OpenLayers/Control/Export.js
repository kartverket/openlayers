    ...
    steps: {
        ...
        format: {
	        ...
	        var size = this.getSizeOfPoints( points );
                ...
                //tcode += "<option value='image/tiff'>GeoTIFF</option>";
                ...
                var updateLink = function (evt) { 
                  ...
                  //var BASE_URL="http://nnrite010:8088/cgi-bin/"+layers+"?";
                  var BASE_URL="http://wms.geonorge.no/skwms1/"+service+"?ticket="+NK.ticket+"&";
                  //var BASE_URL="http://openwms.statkart.no/skwms1/"+service+"?";
                  //var BASE_URL="http://skrivte58:8080/geowebcache/service/wms?"; 
                  var WMS_URL;
                  ...
                  if (format == "geojpeg") {
                    ...
                    WMS_URL = "http://www.norgeskart.no/ws/geoimage.py?url=" + encodeURIComponent("http://openwms.statkart.no/skwms1/"+service+"?styles=&service=WMS&request=GetMap&CRS=EPSG:32633&FORMAT=image/jpeg&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.3.0&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox)+"&px="+px+"&py="+py+"&x="+x+"&y="+y;
                  } else {
                  ...
                  var preview = "<img border=0 width='"+prev_x+"px' height='"+hratio+"px'";
                  var bgw = 300;
                  var bgh = (prev_x > hratio) ? 213 : 400;
                  if (a4) {
                    var padl = (bgw - prev_x) / 2;
                    var padt = (bgh - hratio) / 2;
                    preview += "style='position:relative; left:"+padl+"px; top:"+padt+"px' ";
                  }
                  preview += " src='"+BASE_URL+"styles=&service=WMS&request=GetMap&CRS=EPSG:32633&FORMAT="+preview_format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.3.0&WIDTH="+prev_x+"&HEIGHT="+hratio+"&BBOX="+bbox+"'/>";
                  if (a4) {
                    preview = "<div style='background:white; min-width:"+bgw+"px; min-height:"+bgh+"px;'>" + preview + "</div>";
                  }
                  elements.output.innerHTML = "<p>Forh&aring;ndsvisning:<br/><a target='_blank' style='color:#fff' href='"+WMS_URL+"'>"+preview+"<br/>Klikk her for &aring; &aring;pne/laste ned bildet.</a></p>";
                }
                updateLink(null);
