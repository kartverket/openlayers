...
OpenLayers.Control.Export = 
        ...
        format: {
            draw: function () {
                ...
                function updateSizeSelector() {
                  ...
                  var type = (elements.layer.dataType || elements.layer.type);
                  if ((type == 'wms') || (type == 'wcs')) {
                ...
                function updateFormatSelector() {
                  var layerSel = elements.layerSelector.children[1].children[0].value;
                  var layerStr = layerSel.split("/")[0];
                  var layer    = map.getLayer(layerStr);
                  var formats = layer.dataFormats || ["missing"];
                  var type    = layer.dataType || layer.type || "missing";
                  var code = '<td>'+OpenLayers.Lang.translate('Format') + ": </td><td><select id=\"outputFormat\">";
                ...
                function updateLayerSelector() { 
                  ...
                  for (var l in layers) {
                    var name = layers[l].dataName || layers[l].name;
                    var dL = layers[l].dataLayers;
                    if (!!dL && (Object.prototype.toString.call(dL) === '[object Array]')) {
                      for (var m in dL) {
                        code += '<option value="'+layers[l].id+'/'+dL[m]+'">'+name+' - '+dL[m]+'</option>';
                      }
                    } else {
                      code += '<option value="'+layers[l].id+'">'+name+'</option>';
                    }
                  }
                  code += "</select></td>";
                ...
                function updateLink() { 
                  ...
                  var type = elements.layer.dataType || elements.layer.type;
                  var pscale, size, width, height, preview_format, prev_x, hratio;
                  if ((type == "wms")||(type == "wcs")) {
                    pscale = elements.sizeSelector.children[1].children[0].value;
                    size = elements.that.getSizeOfPoints(points);
                    width   = size[0] * pscale;
                    height  = size[1] * pscale;
                    preview_format = format;
                    prev_x  = Math.min(300, width );
                    hratio  = Math.floor(prev_x * size[1] / size[0]);
                  }
 ...
 /*                   if (!NK.ticket || (NK.ticketedService != service) || (new Date().getTime() - NK.ticketIssued > 3600000 )) {
                    ...
                    if (elements.layer.version == '1.1.1') {
                      var WMS_URL = BASE_URL+"?service=WMS&request=GetMap&SRS="+map.projection.projCode+"&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layer+"&VERSION=1.1.1&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox;
                      var preview = "<img border=0 style='background-color:#fff' width="+prev_x+" height="+hratio+" src='"+BASE_URL+"?service=WMS&request=GetMap&SRS="+map.projection.projCode+"&FORMAT="+preview_format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layer+"&VERSION=1.1.1&WIDTH="+prev_x+"&HEIGHT="+hratio+"&BBOX="+bbox+"'/>";
                    } else {
                    ...
                    linkCode = "<td colspan=2>"+OpenLayers.Lang.translate("Preview")+":<br/><a target='_blank' style='color:#fff' href='"+WMS_URL+"'>"+preview+"<br/>"+OpenLayers.Lang.translate("Download dataset from source")+"</a>";
                    if (!!elements.layer.attribution) {
                      linkCode += "<br/>"+OpenLayers.Lang.translate("Attribution")+": <a style='color:#fff'  href='"+elements.layer.attribution.href+"'>"+elements.layer.attribution.title+"</a>";
                      linkCode += "<br/>"+OpenLayers.Lang.translate("License")+": "+elements.layer.constraints;
                    }
                    linkCode += "</td>";
                  } else if (type == "wfs") {
                      var typeName = elements.layer.dataLayers || elements.layer.protocol.featureType;
                      var outputFormat = $("#outputFormat").val();
                      var WFS_URL = BASE_URL+"?service=WFS&request=GetFeature&srsName="+map.projection.projCode+"&typeName="+typeName+"&version=1.1.0&BBOX="+bbox+"&outputFormat="+outputFormat;

                      linkCode = "<td colspan=2><a target='_blank' style='color:#fff' href='"+WFS_URL+"'>"+OpenLayers.Lang.translate("Download dataset from source")+"</a>";
                      if (!!elements.layer.attribution) {
                        linkCode += "<br/>"+OpenLayers.Lang.translate("Attribution")+": <a style='color:#fff' href='"+elements.layer.attribution.href+"'>"+elements.layer.attribution.title+"</a>";
                        linkCode += "<br/>"+OpenLayers.Lang.translate("License")+": "+elements.layer.constraints;
                      }
                    ...
                    var coverage = elements.layerSelector.children[1].children[0].value.split("/")[1];
                    var WCS_URL = BASE_URL+'?SERVICE=WCS&VERSION=1.0.0&REQUEST=GetCoverage&FORMAT='+format+'&COVERAGE='+coverage+'&BBOX='+bbox+'&CRS='+map.projection.projCode+'&WIDTH='+width+'&HEIGHT='+height;
                    linkCode = "<a target='_blank' style='color:#fff' href='"+WCS_URL+"'>"+OpenLayers.Lang.translate("Download dataset from source")+"</a>";
                    if (elements.layer.dataFormats.indexOf("image/png")>0) {
                      var preview_URL = BASE_URL+'?SERVICE=WCS&VERSION=1.0.0&REQUEST=GetCoverage&FORMAT=image/png&COVERAGE='+coverage+'&BBOX='+bbox+'&CRS='+map.projection.projCode+'&WIDTH='+prev_x+'&HEIGHT='+hratio;
                      var preview = "<img border=0 style='background-color:#fff' width="+prev_x+" height="+hratio+" src='"+preview_URL+"'/>";
                      linkCode = OpenLayers.Lang.translate("Preview") + ":<br/>" + "<a target='_blank' style='color:#fff' href='"+WCS_URL+"'>" + preview + "</a><br/>" + linkCode;
                    }
                    if (!!elements.layer.dataAttribution) {
                      linkCode += "<br/>"+OpenLayers.Lang.translate("Attribution")+": "+elements.layer.dataAttribution;
                      linkCode += "<br/>"+OpenLayers.Lang.translate("License")+": "+elements.layer.dataConstraints;
                    }
                    linkCode = "<td colspan=2>"+linkCode+"</td>";
                  } else {
                    linkCode = "<td colspan=2>"+OpenLayers.Lang.translate("Missing download support for layer type")+" "+(type || "unknown")+"</td>";
                  }
	...
	*/
	...
	var size = self.getSizeOfPoints( points );
        ...
        wrapper.innerHTML += "<br/>"+Math.round(size[0])+"x"+Math.round(size[1]);
    },
