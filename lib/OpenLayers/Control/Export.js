    ...
    steps: {
        ...
        format: {
	        ...
	        var size = this.getSizeOfPoints( points );
                ...
                code += "<option value='image/svg%2Bxml'>SVG</option>";
                code += "<option value='application/x-pdf'>PDF</option>";
                code += "<option value='image/tiff'>GeoTIFF</option>";
                ...
                var updateLink = function (evt) { 
                  ...
                  var BASE_URL="http://nnrite010:8088/cgi-bin/mapserv?map=/opt/maps/topo2/mapfiles/topo2.tmp.map&";
                  //var BASE_URL="http://openwms.statkart.no/skwms1/"+service+"?";
                  var WMS_URL;
                  if ((format == "image/tiff") || (format=="application/x-pdf")) {
                  ...
                  if (format == "geojpeg") {
                    ...
                    WMS_URL = "http://www.norgeskart.no/ws/geoimage.py?url=" + encodeURIComponent("http://openwms.statkart.no/skwms1/"+service+"?styles=&service=WMS&request=GetMap&CRS=EPSG:32633&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.3.0&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox)+"&px="+px+"&py="+py+"&x="+x+"&y="+y;
                  } else {
                    WMS_URL = BASE_URL+"styles=&service=WMS&request=GetMap&SRS=EPSG:32633&FORMAT="+format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.1.1&WIDTH="+width+"&HEIGHT="+height+"&BBOX="+bbox;
                  }
                 
                  var preview = "<img border=0 width="+prev_x+" height="+hratio+" src='"+BASE_URL+"styles=&service=WMS&request=GetMap&CRS=EPSG:32633&FORMAT="+preview_format+"&BGCOLOR=0xFFFFFF&TRANSPARENT=TRUE&LAYERS="+layers+"&VERSION=1.3.0&WIDTH="+prev_x+"&HEIGHT="+hratio+"&BBOX="+bbox+"'/>";
