...
OpenLayers.Control.Print =
  OpenLayers.Class(OpenLayers.Control, {
    ...
    getBiSone: function(geometry, epsg){
      "use strict";
      var lonLatBL = new OpenLayers.LonLat(geometry.bottom, geometry.left).transform(self.map.getProjectionObject(), new OpenLayers.Projection('EPSG:4326'));
      var epsgBL = OpenLayers.Projection.getUTMZone(lonLatBL.lon, lonLatBL.lat).localProj;
      if (epsgBL !== epsg)
        return epsgBL;

      var lonLatTL = new OpenLayers.LonLat(geometry.top, geometry.right).transform(self.map.getProjectionObject(), new OpenLayers.Projection('EPSG:4326'));
      var epsgTL = OpenLayers.Projection.getUTMZone(lonLatTL.lon, lonLatTL.lat).localProj;
      if (epsgTL !== epsg)
        return epsgTL;

      var lonLatBR = new OpenLayers.LonLat(geometry.bottom, geometry.right).transform(self.map.getProjectionObject(), new OpenLayers.Projection('EPSG:4326'));
      var epsgBR = OpenLayers.Projection.getUTMZone(lonLatBR.lon, lonLatBR.lat).localProj;
      if (epsgBR !== epsg)
        return epsgBR;

      var lonLatTR = new OpenLayers.LonLat(geometry.top, geometry.right).transform(self.map.getProjectionObject(), new OpenLayers.Projection('EPSG:4326'));
      var epsgTR = OpenLayers.Projection.getUTMZone(lonLatTR.lon, lonLatTR.lat).localProj;
      if (epsgTR !== epsg)
        return epsgTR;
    ...
    sendPrint: function (geometry) {
        ...
        var biSone = this.getBiSone(geometry, epsg);
