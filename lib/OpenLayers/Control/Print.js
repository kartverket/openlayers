    ...
    steps: {
      format: {
        draw: function () {
          ...
          + '  <label for="ol-print-scale">' + OpenLayers.Lang.translate('freeprint_scale') + ' 1:5 000</label>'
          + '  <select class="form-control, hidden" id="ol-print-scale">'
          + '    <option value="5000">1:5 000</option>'
          + '  </select>'
          + '</div>'
          + '<div class="form-group form-group-sm">'
          + '  <label for="ol-print-title">' + OpenLayers.Lang.translate('freeprint_name_your_map') + '</label>'
          + '  <input type="text" class="form-control" id="ol-print-title" placeholder="Fargelegg (maks. 35 tegn)" maxlength="35">'
    ...
    sendPrint: function (geometry) {
        ...
        var params = {
          map: {
            ...
            ,dpi: '300'
            ,layers: [{
              baseURL: 'http://nnrite008/cgi-bin/fargeleggingskart'
              ,customParams: {
                'TRANSPARENT': 'false'
              }
          ...
          ,titel: document.getElementById('ol-print-title').value || ''
          ,link: refUrl
        };
