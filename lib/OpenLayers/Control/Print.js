...
OpenLayers.Control.Print = 
    ...
    draw: function () {
        ...
        OpenLayers.Event.observe(btn, 'click', OpenLayers.Function.bind(self.togglePrint, self));
    ...
    goPage: function (e) {
        var href = e.target.href.split('#').shift();
        e.target.href = href + window.location.hash;
    },//goPage
    ...
    getContent: function (holder) {
        ...
        btnPrint.setAttribute('class', 'btn btn-success btn-block');
        btnPrint.innerHTML = OpenLayers.Lang.translate('Print');
        
        var btnGo = document.createElement('a');
        btnGo.setAttribute('class', 'btn btn-info btn-block');
        btnGo.setAttribute('href', '/freeprint/');
        btnGo.innerHTML = 'Lag ditt eget turkart!';
        
        content.appendChild(btnPrint);
        content.appendChild(btnGo);
        holder.appendChild(content);
        
        OpenLayers.Event.observe(btnPrint, 'click',
            OpenLayers.Function.bind(self.printPage, self)
        );
        OpenLayers.Event.observe(btnGo, 'click',
            OpenLayers.Function.bind(self.goPage, self)
        );
    },//getContent
