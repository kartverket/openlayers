  ...
  OpenLayers.Class(OpenLayers.Control, {
    ...
    steps: {
      format: {
        draw    : function () {
          ...
          elements.annet = document.createElement('div');
          elements.annet.className = 'col-xs-12 col-sm-12 col-md-12 col-kg-12';
          elements.annet.innerHTML = '<div class="form-group"><label for="ownheader">Velg tittel</label><input class="form-control" type="text" id="ownheader" placeholder="Turkart" value="" /></div>';
          table.appendChild(elements.annet);
          
          elements.scales = document.createElement('div');
          elements.scales.className = 'col-xs-12 col-sm-12 col-md-12 col-kg-12';
          elements.scales.innerHTML = '<div>Velg målestokk</div><div class="btn-group btn-group-justified" data-toggle="buttons"><label class="btn btn-info"><input type="radio" name="scaling" value="25000"> 1 / 25.000</label><label class="btn btn-info active"><input checked="checked" type="radio" name="scaling" value="50000"> 1 / 50.000</label></div>';
          table.appendChild(elements.scales);
          
          elements.legend = document.createElement('div');
          elements.legend.className = 'col-xs-12 col-sm-12 col-md-12 col-kg-12';
          elements.legend.innerHTML = '<div class="checkbox"><label><input type="checkbox" id="legendAccept" value="legendAccepted"> Legg til tegnforklaring</label></div>';
          table.appendChild(elements.legend);
          
          elements.link = document.createElement('div');
          elements.link.className = 'col-xs-12 col-sm-12 col-md-12 col-kg-12';
          elements.link.setAttribute('id', 'testLink');
    ...
    sendPrint: function (geometry) {
        ...
        var params = {
            map: {
                ...
                ,layers: [{
                    ...
                    ,imageFormat: 'image/png' //layer.format //"image/png; mode:24bit"
                    ,layers: ['toporaster'] //[printConfig[layerName]]
                    ,opacity: 1
                    ,type: 'WMS'
                }]
                ,projection: 'EPSG:32633' //epsg
            }
            ,paging: 12
            ,layout: 'A4 landscape'
            ,scale: 35000
            ,titel: document.getElementById('ownheader').value || 'Turkart'
            ,legend: document.getElementById('legendAccept').checked || false
            ,link: location.href
        ...
        $.ajax({
            ...
            document.getElementById('nk-selected-coverage-map_contentDiv').innerHTML = 'Error!';
