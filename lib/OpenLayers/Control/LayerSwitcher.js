        ...
        visDiv.fancytree({
          ...
          dnd: {
            ...
            dragDrop :  function(node, data) {
              var thisLayer = map.getLayer(node.key);
              var thatLayer = map.getLayer(data.otherNode.key);
        ...
        for(var i=0, len=layers.length; i<len; i++) {
            ...
            if (layer.displayInLayerSwitcher) {
                if (layer.getVisibility()) {
                  visTree.addChildren({"title": layer.name, "tooltip":layer.url, "key":layer.id});
                  avlTree.addChildren({"title": layer.name, "tooltip":layer.url, "selected":true, "key":layer.id});
                } else {
                  avlTree.addChildren({"title": layer.name, "tooltip":layer.url, "selected":false, "key":layer.id});
                }
            }
        }
