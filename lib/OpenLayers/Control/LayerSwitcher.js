    ...
    redraw: function() {
        ...
        visDiv.fancytree({
          ...
          dnd: {
            ...
            dragDrop :  function(node, data) {
              var thisLayer = map.getLayersByName(node.title)[0];
              var thatLayer = map.getLayersByName(data.otherNode.title)[0];
              if (data.hitMode == "before") {
                map.setLayerIndex(thisLayer, map.getLayerIndex(thatLayer));
              } else {
                map.setLayerIndex(thisLayer, map.getLayerIndex(thatLayer)+1);
              }
            }
          }
        });
        avlDiv.fancytree({
          ...
          select: function(event, data) {
            ...
            map.getLayersByName(data.node.title)[0].setVisibility(data.node.selected);
          }
        });
        
        var visTree = visDiv.fancytree("getTree").getRootNode();
        var avlTree = avlDiv.fancytree("getTree").getRootNode();
        ...
        for(var i=0, len=layers.length; i<len; i++) {
            ...
            if (layer.displayInLayerSwitcher) {
                if (layer.getVisibility()) {
                  visTree.addChildren({"title": layer.name, "tooltip":layer.url});
                  avlTree.addChildren({"title": layer.name, "tooltip":layer.url, "selected":true});
                } else {
                  avlTree.addChildren({"title": layer.name, "tooltip":layer.url, "selected":false});
                }
            }
        }
