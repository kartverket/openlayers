...
OpenLayers.Control.HeightProfile = OpenLayers.Class(OpenLayers.Control, {
	...
	uploadGPX: function (gpx, callback, errorHandler) {
		var self,
			request,
			requestParams;
        ...
        request = $.ajax({
            ...
            crossDomain: true,
   			...
   			error: function () {
   				...
   				this.getHeightProfileImageUrl(prompt('GPX-filserveren er ikke i drift, så ruten har ikke blitt lagret.\nVisning av høydeprofildata kan testes ved å skrive inn en url til en GPX-fil her.'));
   			}
        }); // request
	},
