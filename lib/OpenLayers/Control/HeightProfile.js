...
OpenLayers.Control.HeightProfile = OpenLayers.Class(OpenLayers.Control, {
	...
	featureAddedHandler: function (evt) {
		...
		featureClone.geometry.transform(this.map.projection, new OpenLayers.Projection("EPSG:4326"));
        ...
        request = $.ajax({
            ...
            crossDomain: true,
   			...
   			complete: function (xml, status, jqXHR) {
   				...
   				console.log('gpx storage server is not available, continuing with dummy data');
	...
	showError: function () {
		this.errorMessageElement = this.errorMessageElement || document.createElement('div');
		this.errorMessageElement.innerHTML = OpenLayers.Lang.translate('An error occured during height profile generation.');
    ...
    draw: function () {
        ...
        this.linestringForm = document.createElement('form');
        this.linestringForm.innerHTML = '<label for="linestring-input">' + OpenLayers.Lang.translate('Use some other format') + '</label><input type="text" name="linestring" id="linestring-input"/><button type="submit">' + OpenLayers.Lang.translate('Show height profile') + '</button>';
        this.cnt.appendChild(this.linestringForm);
