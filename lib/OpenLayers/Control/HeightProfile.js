...
OpenLayers.Control.HeightProfile = OpenLayers.Class(OpenLayers.Control, {
  ...
  getFeatureAsWGS84: function () {
    var featureClones = new Array();
    var i,j;
    if (!this.features && !!this.feature) {
      this.features = [this.feature];
    }
    for (i = 0, j = this.features.length; i < j; i += 1) {
      featureClones[i] = this.features[i].clone();
      featureClones[i].geometry.transform(this.map.projection, this.wgs84Projection);
    }
    this.features = null;
    return featureClones;
  },
  ...
  showProfileLightbox: function () {
    ...
    self.profileLBCloseBtn.innerHTML = OpenLayers.Util.hideFromOldIE('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" class="icon close" preserveAspectRatio="xMidYMid meet"><rect x="1.5" y="1.5" width="33" height="33" /><path class="cross" d="M9,9l18,18M27,9l-18,18"/></svg>') + '<span>' + OpenLayers.Lang.translate('Close') + '</span>';
    ...
    self.profileLBWidget.innerHTML =
      ...
      '<div class="header">' +
      '<h3>' + OpenLayers.Lang.translate('Your height profile') + '</h3>' +
  ...
  draw: function () {
    ...
    this.cnt.innerHTML = '<h3>' + OpenLayers.Lang.translate('Make a height profile') + '</h3>';
    ...
    if (this.fileAPISupported) {
      ...
      this.viewProfileForm.innerHTML = '<label for="gpx-file-upload" class="btn btn-sm btn-primary">' + OpenLayers.Lang.translate('Upload GPX file') + '</label>';

      this.fileInputWrapper = document.createElement('div');
      this.fileInputWrapper.setAttribute('class', 'fileUpload popular-route');

      //this.fileText = document.createElement('span');
      //this.fileText.innerHTML = OpenLayers.Lang.translate('Choose File');
      ...
      //this.fileInputWrapper.appendChild(this.fileText);
      this.fileInputWrapper.appendChild(this.fileInput);
      this.viewProfileForm.appendChild(this.fileInputWrapper);
    ...
    this.wktInput.setAttribute('class', 'form-control input-sm');
    ...
    if (this.gpxTracks && this.gpxTracks.length > 0) {
      ...
      this.gpxTracksElement.setAttribute('class', 'btn-group');
      ...
      for (i = 0, j = this.gpxTracks.length; i < j; i += 1) {
        ...
        OpenLayers.Element.addClass(element, 'popular-route btn btn-sm btn-block btn-primary');
    ...
    this.submitButton.setAttribute('class', 'btn btn-sm btn-primary');
    this.submitButton.disabled = true;
  ...
  drawTrack: function (features) {
    ...
    this.zoomToBoundsOffset(this.mapContainsTrack(features[0]), -1);
    this.submitButton.disabled = false;
    this.wktInput.value = null;
  },
  ...
  showControls: function () {
    ...
    this.activate();
    $(this.widget).css('top', ($(this.div).position().top * -1));
  }, // showControls
