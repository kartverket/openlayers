...
OpenLayers.Control.HeightProfile = OpenLayers.Class(OpenLayers.Control, {
  ...
  draw: function () {
    ...
    if (this.fileAPISupported) {
      ...
      this.viewProfileForm.innerHTML = '<label for="gpx-file-upload" class="btn btn-primary">' + OpenLayers.Lang.translate('Upload GPX file') + '</label>';
    ...
    this.wktInput.setAttribute('class', 'form-control');
    ...
    if (this.gpxTracks && this.gpxTracks.length > 0) {
      ...
      for (i = 0, j = this.gpxTracks.length; i < j; i += 1) {
        ...
        OpenLayers.Element.addClass(element, 'popular-route btn btn-primary');
