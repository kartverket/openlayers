...
OpenLayers.Control.Popout = OpenLayers.Class( OpenLayers.Control, {
    ...
    initialize: function(options) {
        ...
        this.events.fallThrough = false;
    },
    
    draw: function () {
        var imgLocation, img, wrapper, anchor;
        ...
        anchor.style.cssText = 'display:block;height:30px;padding:5px;width:30px';
        var testHash = window.location.href.split('#')[0].split('/');
        if (testHash.length > 4) {
            anchor.href = 'http://www.norgeskart.no/' + testHash[3] + '/#' + window.location.href.split('#')[1];
        } else {
            anchor.href = 'http://www.norgeskart.no/#' + window.location.href.split('#')[1];
        }
        //self.link = link;

        wrapper = document.createElement('div');
        wrapper.style.position= 'fixed';
        wrapper.style.background = 'rgba(80,80,80,0.9)';
