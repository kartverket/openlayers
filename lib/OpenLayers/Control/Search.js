...
OpenLayers.Control.Search = OpenLayers.Class(OpenLayers.Control, {
	...
	keyboardBtn.setAttribute('type', 'button');
        keyboardBtn.setAttribute('id', 'keyboardButton');
        keyboardBtn.setAttribute('class', 'keyboard-button');
        keyboardBtn.innerHTML = '<img src="theme/norgeskart/img/keyboard.png" width="19" height="13" alt="K">';
	OpenLayers.Event.observe(keyboardBtn, 'click', OpenLayers.Function.bind(this.toggleKeyboard, this));
	...
	divgroup.appendChild(keyboardBtn);
	...
	$.extend($.keyboard.keyaction, {
		no: function (base) {
			base.position.my = 'left top';
			base.position.at2 = 'left bottom';
    ...
    drawResponse: function() {
        ...
        if (xmlPlaces.length === 0 && json === null) {
            ...
            areasElement.append($('<h2 class="h">område</h2>'));
            areasListElement = $('<ul/>');
            ...
            objectsElement.append($('<h2 class="h">objekt</h2><ul>'));
            ...
            if (!!json){
              $.each(json, function(){
                ...
                var coords = new OpenLayers.LonLat(parseFloat(this['LONGITUDE']), parseFloat(this['LATITUDE']));
                  ...
                  if (coords.lat > 64 && coords.lon < 30){
                    trf.transformDirect(this['LONGITUDE'], this['LATITUDE'], 22, 84, function (tmpCoords) {
                      coords.lat = tmpCoords.nord;
                      coords.lon = tmpCoords.ost;
                    });
                  }
            ...
            xmlPlaces.each(function(){
                ...
                place = {
                    ...
                    stedsnummer: $(this).find("stedsnummer").text(),
    ...
    toggleKeyboard: function() {
        var kb  = $('#searchInput').getkeyboard();
        if (kb.isVisible()) {
            kb.accept();
        } else {
            kb.reveal();
    ...
    addMarkers: function (places) {
        ...
        function generatePopupMarkup (feature) {
            ...
            if (feature.attributes.type) {
                ...
                if (feature.attributes.stedsnummer) {
                    markup += '<div><a target="_blank" href="http://faktaark.statkart.no/?stedsnummer=' + feature.attributes.stedsnummer + '">Faktaark</a></div>';
                }
            } else {
