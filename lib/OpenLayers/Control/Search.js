...
//                 qurl = this.internalUrl + 
        ...
        if (places.length == 0) {
            ...
            places.each(function(){
                ...
                switch(type) {
                    ...
                    case 'Grend':
                        zoomLevel = 8;
                        listItem = '<li class="search-place" data-east="' + east + '" data-north="' + north + '" data-zoom="' + zoomLevel + '">' + place + ', ' + type + '</li>';
    ...
    addMarkers: function (coords) {
        ...
        for(i = 0, j = coords.length; i < j; i += 1) {
            lonlat = new OpenLayers.LonLat(coords[i].east, coords[i].north);
            lonlat.transform(coor_from, coor_to);
            mrk = new OpenLayers.Marker(lonlat,icon.clone());
