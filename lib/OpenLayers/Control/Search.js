...
OpenLayers.Control.Search = OpenLayers.Class(OpenLayers.Control, {
    ...
    draw: function () {
        ...
        $(document).on("click", '.search-place', function(){
            ...
            var ssridActual = parseInt($(this).data('ssrid')) || 0;
            var ssridArr = [];
            var layer = that.getMarkerLayer();
            if (layer !== null) {
                var i, j;
                for (i = 0, j = layer.features.length; i < j; i++) {
                    var ssridTemp = parseInt(layer.features[i].data.ssrid) || 0;
                    if (ssridTemp != ssridActual) {
                        ssridArr.push( ssridTemp.toString() );
                    }
                }
                for (i = 0, j = ssridArr.length; i < j; i++) {
                    var features = layer.getFeaturesByAttribute('ssrid', ssridArr[i]);
        ...
        if (xmlPlaces.length === 0 && json === null) {
            ...
            if (!!json){
              $.each(json, function(){
                ...
                if (!this.hasOwnProperty('LATITUDE') || !this.hasOwnProperty('LONGITUDE')) {
                  ...
                  if (coords.lat > 64 && coords.lon > 18){
                    //note: we transform from UTM32 to WGS84 using SKtrans when the point is actuall in UTM34/35/36, because proj4 doesn't work correctly for this
                  ...
                  placeElement.attr("data-ssrid", place.ssrid);
            ...
            xmlPlaces.each(function(){
                ...
                var getPlaceElement = function (placeData, zoomLevel, contextFunction) {
                    ...
                    element.attr("data-ssrid",  placeData.ssrid);
