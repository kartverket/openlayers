        ...
        if (xmlPlaces.length === 0 && json === null) {
            ...
            if (!!json){
              $.each(json, function(){
                ...
                var data = this;
                //note: we transform from UTM32 to 33 correctly using SKtrans, then use the slightly broken proj4 to transform to/from WGS coordinates repeatedly
                trf.transformDirect(data['LONGITUDE'],data['LATITUDE'],22,23,function(coords) {

                  var point = new OpenLayers.LonLat(coords['ost'], coords['nord']);
                  point.transform(new OpenLayers.Projection("EPSG:32633"), new OpenLayers.Projection("EPSG:4326"));

                  place = {
                    name: data['NAVN'].charAt(0) + data['NAVN'].slice(1).toLowerCase(),
                    ssrid: data['ID'],
                    type: data['OBJEKTTYPE'],
                    municipality: data['KOMMUNENAVN'].charAt(0) + data['KOMMUNENAVN'].slice(1).toLowerCase(),
                    county: data['FYLKESNAVN'].charAt(0) + data['FYLKESNAVN'].slice(1).toLowerCase(),
                    north: point.lat,
                    east: point.lon
                  };
                  ...
                  if (typeof data['HUSNUMMER'] !== 'undefined' && data['HUSNUMMER'].length){
                    ...
                    for (var i = 0; i < data['HUSNUMMER'].length; i++) {
                        option = document.createElement('option');
                        if (document.all) {
                            option.innerText = data['HUSNUMMER'][i];
                        } else {
                            option.textContent = data['HUSNUMMER'][i];
                        }
                        select.appendChild(option);
                    }
