...
OpenLayers.Control.Search = OpenLayers.Class(OpenLayers.Control, {
    ...
    streetAddressSearchUrl: 'http://beta.norgeskart.no/ws/veg.py',
    addressSearchUrl: 'http://beta.norgeskart.no/ws/adr.py',
    houseSearchUrl: 'http://beta.norgeskart.no/ws/eie.py',
    ...
    drawResponse: function(xml) {
        ...
        if (places.length === 0) {
            ...
            if (listAddress.length > 0) {
                ...
                for (i = 0, j = addresses.length; i < j; i += 1) {
                    var a = addresses[i];
                    $.ajax({
                        'url': that.streetAddressSearchUrl, 
        ...
        $('#searchResults').on(
            ...
            function (evt) { 
                ...
                $this = $(this);
                place = $this.parent('.search-place');
