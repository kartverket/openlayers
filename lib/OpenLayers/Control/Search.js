...
//        size = new OpenLayers.Size( 230, 58);
    ...
    getAddressResultHandler: function (DOMElementId) {
        var handler = function (response) {
            ...
            addressElement = OpenLayers.Util.getElement(DOMElementId);
            if (parsedResponse.length > 0 && parsedResponse[0].HUSNUMMER) {
                ...
                if (numbers.length > 1) {
                    ...
                    if ($.uniform) {
                        $(addressElement).find('select').uniform();
                    }
                } else {
    ...
    drawResponse: function(xml) {
        ...
        if (places.length === 0) {
            ...
            places.each(function(){
                ...
                switch(place.type) {
                    ...
                    case 'Adressenavn (veg/gate)':
                        ...
                        listItem = '<li class="search-place" id="' + addressId + '" data-east="' + place.east + '" data-north="' + place.north + '" data-zoom="' + zoomLevel + '"><span class="street-address"><span class="street-name">' + place.name + '</span></span>, <span class="municipality-name">' + place.municipality + '</span></li>';
            ...
            if (totalResultsReported) {
                resultsPerPage = parseInt($('#inputResultsPerPage').val());
                page = parseInt($('#inputResultPageNumber').val());
        ...
        $('#searchResults select').on('click', function(evt) {evt.stopPropagation();});
        ...
        $('#searchResults').on(
            ...
            function (evt) { 
                ...
                queryString = '';
                if (streetElement) {
                    queryString += streetElement.textContent + ' ' + $this.val().toString() + ', ' + municipalityElement.textContent; 
                }
                $.ajax({
    ...
    addMarkers: function (places) {
        ...
        function generatePopupMarkup (feature) {
            ...
            if (feature.attributes.type) {
                ...
                markup += '<div><a href="http://faktaark.statkart.no/SSRFakta/faktaarkfraobjektid?enhet=' +feature.attribute.ssrid + '">Faktaark</a></div>';
            } else {
