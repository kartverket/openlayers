...
OpenLayers.Control.Search = OpenLayers.Class(OpenLayers.Control, {
    ...
    //searchUrl: '/ssrws/ssr/sok',
    searchUrl: 'https://ws.geonorge.no/SKWS3Index/ssr/sok',
    streetAddressesSearchUrl: '/ws/veg.py',
    addressSearchUrl: '/ws/adr.py',
    propertySearchUrl: '/ws/eie.py',
    resultsPerPage: 15,
    markerLayer: null,
    menu : null,
    timer : null,
...
//        size = new OpenLayers.Size( 230, 58);
        ...
        $(document).on("keyup", '#searchInput', function(e) {
                ...
                if ( resultsList.length === 0 ) {
                    resultsList = $( '<div/>', { 'id': 'searchResults', html : '<a href="#" class="close">lukk</a>'});
                    $('.searchDiv').after(resultsList);
                }

                resultsList.html('<a href="#" class="close">lukk</a>');
        ...
        */
        ...
        var searchEngine = {
            ...
            run: function (params, search) {
                ...
                var request,
                    requestParams = {}, listOfParams;

                listOfParams = params.phrase.split(',');

                if(listOfParams.length > 1 ) {
                    requestParams[this.paramNames.name] = listOfParams[0];
                    listOfParams[1] = listOfParams[1].trim();
                    listOfParams[1] = listOfParams[1].substring(0, listOfParams[1].length - 1);
                    var testNumber = parseInt(listOfParams[1]);
                    testNumber = (testNumber !== testNumber);
                    if(!testNumber) {
                        requestParams[this.paramNames.fylkeKommuneListe] = listOfParams[1];
                    } else {
                        requestParams[this.paramNames.fylkeKommuneListe] = '';
                    }
                } else {
            ...
            run: function (params, search) {
                ...
                request.done(function(response){
                    ...
                    if(response.length !== 0) {
                        ...
                        search.searchData.gbrnr = response;
                        search.drawResponse();
                    }
                });// done
    ...
    */
    drawResponse: function() {
        ...
        if (json !== null && json.length === 0) {
            if(this.searchData.address !== null && this.searchData.address.length  !== 0) {
                json = this.searchData.address;
            }
        } else if(json === null) {
            json = this.searchData.address;
            if(this.searchData.gbrnr !== null) {
        ...
        if (xmlPlaces.length === 0 && json === null) {
            ...
            */
            ...
            objectsElement.append($('<h2 class="h">stedsnavn</h2><ul>'));
            ...
            $.each(json, function () {
                ...
                if (!this.hasOwnProperty('LATITUDE') || !this.hasOwnProperty('LONGITUDE')) {
                    ...
                    if (typeof this['HUSNUMMER'] !== 'undefined' && this['HUSNUMMER'].length) {
                    ...
                    } else if (typeof this['HUSNR'] !== 'undefined' && this['HUSNR'].length) {
                        option.value = '';
                        ...
                        for (var i = 0; i < this['HUSNR'].length; i++) {
                            option = document.createElement('option');
                            if (document.all) {
                                option.innerText = this['HUSNR'][i];
                            } else {
                                option.textContent = this['HUSNR'][i];
                            }
                            select.appendChild(option);
                        }
                        placeElement.append(select);
        ...
        */
        ...
        /*
            Handler for housenumberrch select menu
        */
        resultsList.on(
            'change', 
