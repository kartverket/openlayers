...
//        size = new OpenLayers.Size( 230, 58);
    ...
    parseInput: function (input) {
        ...
        decimalPairDot   = /^[ \t]*([0-9]+\.[0-9]+|[0-9]+)(?:[ \t]+,|,)[ \t]*([0-9]+\.[0-9]+|[0-9]+)[ \t]*$/; // hir -- test: fix input like "0520" without comma.
    ...
    doSearch: function (phraseParameter, page, pageLength) {
        ...
        */
        ...
        if (params) {
	        ...
	        var trf = map.getControlsByClass("OpenLayers.Control.Transformations")[0];
                ...
                if (params.north) {
                    ...
                    var inputEPSG = trf.getEPSGfromSOSI(inputProj);
                    if (!trf.isViewable(inputEPSG)) {
                      var inputProj = "23";
                      var inputEPSG = trf.getEPSGfromSOSI(inputProj);
                    }
                    var fromProj = proj[inputEPSG];
                ...
                var inputName = trf.getCRSName(inputEPSG);
                var toProj   = new OpenLayers.Projection(this.map.getProjection());
                center.transform(fromProj, toProj);
                this.map.setCenter(center);
                this.addMarkers([params]);

                // TODO: display alternate projections here
                var coordinateSystemSelector = trf.generateCoordinateSystemsList('coordinate-system-selector', OpenLayers.Lang.translate('Coordinate system:'), inputProj, true);
                ...
                resultsList.html( $('<div/>', {'id':'projectionNotice', html: 'Koordinatene tolkes som <a href="http://epsg.io/' + inputEPSG + '" style="color:#fff">' + inputName + "</a>. Bytt projeksjon til "}) );
                document.getElementById("projectionNotice").appendChild(coordinateSystemSelector.select); // huh, shouldn't there be a direct way to do this?
                resultsList.show();
    ...
    changeProjection: function (proj) {
      map.getControlsByClass('OpenLayers.Control.Transformations')[0].setCoordinateCookie(proj, false); 
      this.doSearch();
    },
