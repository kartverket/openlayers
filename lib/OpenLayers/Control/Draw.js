...
OpenLayers.Control.Draw = OpenLayers.Class(OpenLayers.Control, {
        ...
        handlers: {
            ...
            download: function (evt) {
              if (!!this.savedState.url) {
                window.open(this.savedState.url);
              };
            },
            undo: function (evt) {
                var feature,
                ...
                request = $.ajax({
                    ...
                    success: function (response, statusText, jqXHR) {
                        ...
                        if (response.indexOf('http') === 0) {
                            ...
                            this.downloadButton.setAttribute('style','color:#fff');
    ...
    draw: function () {
        ...
        toolItem = document.createElement('li');
        buttonText = OpenLayers.Lang.translate('Download');
        var button = this.tools.buttons['download'] = document.createElement('button');
        button.setAttribute('title', OpenLayers.Lang.translate('Download drawing in GeoJSON format. More options to come.'));
        OpenLayers.Element.addClass(button, 'fileUpload popular-route');
        if (!this.tools.icons['download']) {
            OpenLayers.Element.addClass(toolItem, 'text-only');
        }
        buttonContent = '<span>'+buttonText+'</span>';
        button.innerHTML = buttonContent;
        ...
        if (!this.savedState.url) {
          button.setAttribute('style','color:#444');
        }
        OpenLayers.Event.observe(button, 'click', OpenLayers.Function.bind(this.tools.handlers['download'], self));
    ...
    drawTrack: function (features) {
      ...
      var dataExtent = features[0].geometry.bounds;
      if (!map.getExtent().containsBounds(dataExtent)) {
