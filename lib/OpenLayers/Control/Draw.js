...
OpenLayers.Control.Draw = OpenLayers.Class(OpenLayers.Control, {
  ...
  showLabelEditorPopup    : function (evt) {
    ...
    if (!feature.popup) {
      ...
      popup.input.setAttribute('class', 'form-control');
      ...
      popup.okButton.setAttribute('class', 'update-feature-label btn btn-primary');
      ...
      popup.cancelButton.setAttribute('class', 'cancel-feature-label btn btn-primary');
  ...
  tools                   : {
    ...
    handlers: {
      ...
      save   : function (evt) {
        ...
        + '<p>' + OpenLayers.Lang.translate('tegneogmale_lagrer_tegning') + '</p><p>' + OpenLayers.Lang.translate('vennligst_vent') + '</p>'
        ...
        request = $.ajax({
          ...
          success    : function (response, statusText, jqXHR) {
            ...
            if (response.indexOf('http') === 0) {
              ...
              // Check if the returned url contains a 40 digit hexadecimal SHA1-hash.
              var res = url.match(/[0-9a-f]{40}/g);
              // If hash is present, build binary link, else use the returned url.
              var downloadUrl = res.length == 0 ? url : 'http://www.norgeskart.no/ws/get-json.py?hash=' + res[0] + '&save=true';
              
              // GeoJSON successfully saved. Updating save modal with info and download url:
              document.getElementById('overlay').innerHTML = '<div class="content">'
              + '<p><strong>' + OpenLayers.Lang.translate('tegneogmale_lagret') + '</strong></p>'
              + '<p>' + OpenLayers.Lang.translate('tegneogmale_lagret_fil_info') + '</p>'
              + '<p><code style="display: block; word-break: break-all;">' + window.location.href + '</code></p>'
              + '<p><a class="btn btn-sm btn-success btn-block" href="' + downloadUrl + '" target="_blank">' + OpenLayers.Lang.translate('tegneogmale_last_ned') + '</a></p>'
              + '<button aria-label="Close" class="close" onclick="document.getElementById(\'overlay\').style.display = \'none\'" style="position: absolute;" type="button"><span aria-hidden="true">×</span></button>'
              + '</div>';
            }
          },
          error      : function () {
  ...
  draw                  : function () {
    ...
    this.cnt.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Drawing') + '</h3>';
  ...
  drawMeasuresSection: function () {
    ...
    wrapper.innerHTML = '<h3 class="h">' + OpenLayers.Lang.translate('Measures') + '</h3>'
