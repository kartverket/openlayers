...
OpenLayers.Control.Draw = OpenLayers.Class(OpenLayers.Control, {
    ...
    tools: {
        icons: {
            ...
            label: '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="100px" height="100px" preserveAspectRatio="xMidYMid meet" viewBox="0 0 100 100" class="icon text"><path d="M 0,0 100,0 100,30 90,20 60,20 60,90 70,100 30,100 40,90 40,20 10,20 0,30 Z" /></svg>'
        },
        handlers: {
            point: function (evt) {
            ...
            rectangle: function (evt) { /* hidden on interface */
                this.deactivateDrawControls();
                this.drawControls.rectangle.activate();
                //this.drawControls.rectangle.handler.layer.events.register('featureadded', this, this.allowMeasuresUpdating);
                //OpenLayers.Element.addClass(this.tools.buttons.rectangle, 'active');
                //this.activeDrawControl = 'rectangle';
                this.resetMeasures();
                this.showAreaMeasures();
            },
            label: function (evt) {
    ...
    sketchComplete: function(feature) {
          //only relevant if drawing rectangles and XDM is enabled atm
          var feature = feature.feature;
          var g = feature.geometry;
          if (g.CLASS_NAME == "OpenLayers.Bounds") {
            var lt = map.getLonLatFromPixel(new OpenLayers.Pixel(g.left, g.top));
            var rb = map.getLonLatFromPixel(new OpenLayers.Pixel(g.right, g.bottom));
            g.left = lt.lon; g.right = rb.lon; g.top = lt.lat; g.bottom = rb.lat;
            feature.geometry = g.toGeometry();
            this.displayBBoxFeature(feature, true);
          } else {
            this.displayBBoxFeature(feature, false);
          }
          ...
          if (NK.xdm) {
            NK.functions.postMessage({
              ...
              "feature":JSON.parse(geojson) // GeoJSON.extract.geometry is buggy
            });
          }
    },
    displayBBoxFeature: function(feature, modifiable) {
            this.layer.removeAllFeatures();
            if (!!this.modifyControl) {
              this.modifyControl.deactivate();
              map.removeControl(this.modifyControl);
            }
            ...
            if (NK.xdm) {
              ...
              this.layer.events.on({"featuremodified": function() {
                var geojson = format.write(feature);
                NK.functions.postMessage({
    ...
    setMap: function (map) {
        ...
        layerOptions = {'visibility': false, 'projection':map.projection};
        if (this.styleMap) {
        ...
        this.drawControls.rectangle = new OpenLayers.Control.DrawFeature(this.layer, OpenLayers.Handler.Box, drawControlOptions);
        ...
        this.layer.events.register('sketchcomplete', this, this.sketchComplete);
    ...
    activateTool: function(tool) {
      this.tools.handlers[tool].apply(this, []);
    },
