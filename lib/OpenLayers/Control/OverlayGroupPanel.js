...
OpenLayers.Control.OverlayGroupPanel = 
    ...
    initialize: function(options) {
        ...
        if (options.buttonIcon) {
            this.btnIcon = options.buttonIcon;
        }
    ...
    updateLayer: function (layerData) {
        ...
        var layerGroupTest = layerData.layer.layerGroup.split('.')[0];
        for (var i = 0; i < this.map.layers.length; i += 1) {
          if (!!this.map.layers[i].layerGroup ) {
            if ((this.map.layers[i].layerGroup !== layerData.layer.layerGroup) && (OpenLayers.String.startsWith(this.map.layers[i].layerGroup,layerGroupTest))) {
              this.map.layers[i].setVisibility(0);
            }
            if ((this.map.layers[i].layerGroup === 'tjenester.alle') &&(this.map.layers[i].shortid !== layerData.layer.shortid) ){
              this.map.layers[i].setVisibility(0);
              for (layer in this.layers) {
                  if (this.layers[layer].layer.id === this.map.layers[i].id){
                    OpenLayers.Element.removeClass(this.layers[layer].link, 'active');
                  }
              }
            }
          }
    ...
    draw: function () {
      ...
      var tmp = document.createElement('div');
        ...
        if (this.btnIcon) {
            buttonContent = '<img src="' + this.btnIcon + '" class="btnIcon"></img>';
        }
    ...
    showControls: function () {
      for (var i = 0; i < this.map.controls.length; i += 1) {
        if (this.map.controls[i] instanceof OpenLayers.Control.OverlayGroupPanel) {
          if (OpenLayers.Element.hasClass( map.controls[i].div, 'active' )){
            this.map.controls[i].hideControls();
          }
        }
      }
      this.updateStatus();
