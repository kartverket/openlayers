  ...
  initialize: function(options) {
    ...
    */
                    ...
                    '<div class="location-details-wrapper">'+
                        '<div class="location-details-header">'+
                            ...
                            '<img src="theme/norgeskart/img/KV_nodplakat_2.png" alt="2."><p>'+OpenLayers.Lang.translate('My street is');
                                ...
                                if(!!self.matrikkelInfo.veg && this.streetNameAccept === 1){
                                  ...
                                  posterContent.street = self.matrikkelInfo.veg + ' ' + OpenLayers.Lang.translate('in') + ' '  +  self.pointData.kommune +  ' ' + OpenLayers.Lang.translate('municipality');
                                } else {
                                  tmpText += '</br></p>';
                                }
                        ...
                        '</div>'+
                          '<p>' + OpenLayers.Lang.translate('My UTM location') + ' Sone ' + sone + ' ' + Math.round(localCoo.lon)+',&nbsp;'+Math.round(localCoo.lat) +
                            '</br>' + 'Min posisjon i desimalgrader: N' + dd.lat +  ' - E' + dd.lon +
                          '</p>'+
                    '</div><img src="theme/norgeskart/img/KV_nodplakat_footer.png" alt="Kartverket">'+
                '</div>'+
            '</div>';
      posterContent.utm = sone + ' ' + Math.round(localCoo.lon) + ', ' + Math.round(localCoo.lat);
      posterContent.posDez = 'N' + dd.lat +  ' - E' + dd.lon ;
    ...
    getLocationContent: function (holder) {
        ...
        streetAccept.innerHTML = '<option selected disabled>Er dette riktigt veg?</option><option value="yes">Ja</option><option value="no">Nei</option>';
        streetAcceptContainer.appendChild(streetAccept);

        var tmpText;
        if(!!self.matrikkelInfo.veg ){
          tmpText = '<h2>Funnet veg er: </h2>';
          tmpText += '<h3>' + self.matrikkelInfo.veg;
          if (self.pointData.kommune) {
            tmpText += ' ' + OpenLayers.Lang.translate('in') + ' '  +  self.pointData.kommune +  ' ' + OpenLayers.Lang.translate('municipality');
          }
        ...
        if (self.pointData.kommune) {
          title += ' ' + OpenLayers.Lang.translate('in') + ' ' + self.pointData.kommune + ' ' + OpenLayers.Lang.translate('municipality');
        }
        ...
        container.innerHTML =
            ...
            '<div class="step-body">' +
                '<div class="emergency-point-name-input-wrapper">' +
                    ...
                    '<input type="text" id="emergencyPointNameInput" value="' + self.locationName + '">' +
    ...
    */
    ...
    getDDfromDMS: function(dms){
        ...
        function makeDD(deg, min, sec) {
            return Math.round((deg + (min / 60) + (sec / 3600))*1000) / 1000;
        }
    ...
    previewPrintHandler: function(){
      //window.print();
      var epURL = "http://dbriap016:8080/fop/fop?";
      ...
      epURL += '&matrikkel=' + posterContent.matrikkel;
      epURL += '&utm=' + posterContent.utm;
