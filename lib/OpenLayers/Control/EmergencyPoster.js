...
OpenLayers.Control.EmergencyPoster = OpenLayers.Class(OpenLayers.Control.Panel, {
  ...
  ssrwsURL: 'https://ws.geonorge.no/SKWS3Index/v2/ssr/sok?',
  ...
  initialize: function(options) {
    ...
    drawPointSelectInstruction: function () {
    	...
    	this.pointSelectInstructionWidget = OpenLayers.Util.createWidget(content, 1, true);
    ...
    drawPreviewPopup: function(){
      ...
      OpenLayers.Element.addClass(self.previewPrintButton, 'print btn btn-primary');
    ...
    getLocationContent: function (holder) {
        ...
        if (self.altPoints.length) {
          ...
          self.altPointList.setAttribute('class', 'form-control');
      ...
      streetAccept.setAttribute('class', 'form-control');
        ...
        var tmpText;
        if(!!self.matrikkelInfo ){
          tmpText = '<h4>Funnet veg er: </h4>';
          tmpText += '<h5>' + self.matrikkelInfo.veg;
          if (self.matrikkelInfo.kommune) {
            tmpText += ' ' + OpenLayers.Lang.translate('in') + ' '  +  self.matrikkelInfo.kommune +  ' ' + OpenLayers.Lang.translate('municipality');
          }
          tmpText += '</h5>' + streetAcceptContainer.outerHTML;
        } else {
          tmpText = '<h4>Ingen veg funnet.</h4>';
        }
        var title;
        title = (!self.pointData.stedsnavn || self.pointData.stedsnavn.length <= 1) ? OpenLayers.Lang.translate('Unknown') : self.pointData.stedsnavn;
        ...
        container.innerHTML =
            ...
            '<div class="step-body">' +
                '<div class="emergency-point-name-input-wrapper">' +
                    ...
                    '<input type="text" id="emergencyPointNameInput" class="form-control required" value="' + self.locationName + '" aria-required="true" placeholder="('+OpenLayers.Lang.translate('E.g. the name of your cabin')+')"/>' +
                '</div>' +
                '<div class="emergency-point-location-wrapper">' +
                    '<h4>'+OpenLayers.Lang.translate('Location is')+'</h4>' +
                    '<h5>'+title+'</h5>' +
      ...
      OpenLayers.Element.addClass(self.locationRedoButton, 'btn btn-default');
      ...
      OpenLayers.Element.addClass(self.locationAcceptButton, "btn btn-primary");
    ...
    getTermsContent: function (holder) {
        ...
        container.innerHTML =
            ...
            '</div>'+
            '<div class="step-body">' +
                '<h4 class="h">'+OpenLayers.Lang.translate('Legal')+'</h4>'+
                '<p>'+
        ...
        OpenLayers.Element.addClass(self.termsAcceptButton, "btn btn-primary");
    ...
    */
     ...
     */
      function makeSquare(center_lon, center_lat, radiusInMiles) {
        ...
        OpenLayers.Util.createAjaxRequest(function (result) {
          ...
          pointDone = true;
          if (boxDone) {
            self.setPointData(self.pointData['stedsnummer']['value']);
    ...
    buildAltPointsList: function(){
        ...
        for (i = 0; i < self.altPoints.length; i++) {
            if (html) {
                html = html + '<option value="'+self.altPoints[i].stedsnummer+'">' + self.altPoints[i].stedsnavn + '</option>';
            } else {
    ...
    parseBoxData: function(xml){
        ...
        for (i = 2; i < places.length; i++) {
            ...
            point = {
                ...
                stedsnummer       : place.childNodes[11].childNodes[0].nodeValue
            };
            self.altPoints.push(point);
        }
    },

    setPointData: function (stedsnummer) {
        var self = this, i, loop;
        if (!stedsnummer && stedsnummer != -1) {
            // invalid stedsnummer
            return;
        }
        if (self.altPoints.length) {
            ...
            for (i = 0; i < loop; i++) {
                if (parseInt(stedsnummer) == parseInt(self.altPoints[i].stedsnummer)) {
                    //match = true;
