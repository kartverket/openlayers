...
OpenLayers.Control.EmergencyPoster = OpenLayers.Class(OpenLayers.Control.Panel, {
    ...
    drawPreviewPopup: function(){
        ...
        var title = (!self.pointData.stedsnavn) ? OpenLayers.Lang.translate('Unknown') : self.pointData.stedsnavn + ' ' + OpenLayers.Lang.translate('in') + ' ' + self.pointData.kommune + ' kommune';
    ...
    getLocationContent: function (holder) {
        ...
        var placename = self.pointData.stedsnavn || OpenLayers.Lang.translate('Unknown');
    ...
    */
    ...
    getPointData: function () {
        ...
        */
        ...
        rect   = makeSquare(lonLat.lon, lonLat.lat, 0.5);
    ...
    setPointData: function(ssrid) {
        ...
        if (!ssrid || !self.altPoints.length) return;
        ...
        for (var i = 0; i < loop; i++) {
            ...
            console.log(ssrid + ' : ' + self.altPoints[i].ssrID + ', ' + self.altPoints[i].stedsnavn);
        ...
        if (!match) {
            // Translate original pointData object into same structure as altPoints
            self.pointData = {
                ssrID       : ssrid,
                kommune     : self.altPoints[0].kommune,
                fylke       : self.altPoints[0].fylke,
                stedsnavn   : self.pointData.placename.value,
                lon         : self.coordinates.lon,
                lat         : self.coordinates.lat,
                skriveMaateNavn : self.pointData.placename.value,
                epsgKode    : self.map.getProjectionObject().getCode()
            };
        }
    },
