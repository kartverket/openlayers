...
OpenLayers.Control.EmergencyPoster = OpenLayers.Class(OpenLayers.Control.Panel, {
    ...
    drawPreviewPopup: function(){
        ...
        var dms = self.getDMSfromCoord(self.epsg);
                ...
                '<div class="preview">'+
                    ...
                    '<div class="location-details-wrapper">'+
                        '<div class="location-details-header">'+
                            ...
                            '<h2>'+
                                dms.lat.bits.deg+' '+OpenLayers.Lang.translate('degrees')+' '+dms.lat.bits.min+' '+OpenLayers.Lang.translate('minutes')+' '+dms.lat.bits.sec+' '+OpenLayers.Lang.translate('seconds')+' nord,'+
                                '</br>'+
                                dms.lon.bits.deg+' '+OpenLayers.Lang.translate('degrees')+' '+dms.lon.bits.min+' '+OpenLayers.Lang.translate('minutes')+' '+dms.lon.bits.sec+' '+OpenLayers.Lang.translate('seconds')+' øst'+
                            '</h2>'+
                        '</div>'+
                        ...
                        '<div class="location-details-coords">'+
                            ...
                            '<table class="emergency-details-coordsinfo">'+
                                ...
                                '<tr>'+
                                    ...
                                    '<td><strong>'+dms.lat.bits.deg+'/'+dms.lat.bits.min+'/'+dms.lat.bits.sec+'</strong></td>'+
                                    '<td><strong>'+dd.lat+'</strong></td>'+
                                    '<td><strong>xx/xx/xx</strong></td>'+
                                '</tr>'+
                                '<tr>'+
                                    '<td>'+OpenLayers.Lang.translate('east')+' (long)</td>'+
                                    '<td><strong>'+dms.lon.bits.deg+'/'+dms.lon.bits.min+'/'+dms.lon.bits.sec+'</strong></td>'+
    ...
    getDDfromDMS: function(dms){
        ...
        if (dms.lat.bits.dir === OpenLayers.Lang.translate('S')) {
            latdd = -Math.abs(latdd);
        }

        londd = makeDD(dms.lon.bits.deg, dms.lon.bits.min, dms.lon.bits.sec);
        if (dms.lon.bits.dir === OpenLayers.Lang.translate('W')) {
            latdd = -Math.abs(latdd);
        }
    ...
    getDMSfromCoord: function(epsg) {
        ...
        return {
            lon : {
                bits : makeBits(Math.abs(epsg.lon)),
                dir  : ((epsg.lon > 0) ? east : west)
            },
            lat : {
                bits : makeBits(Math.abs(epsg.lat)),
                dir : (epsg.lat > 0) ? north : south
            }
        };
