...
OpenLayers.Control.EmergencyPoster = OpenLayers.Class(OpenLayers.Control.Panel, {
  ...
  matrikkelInfoURL: 'http://www.norgeskart.no/ws/emergencyPoster_test.py?',
  pointMenu: null,
  title: null,
  pointSelector: null,
  pointData: null,
  ...
  initialize: function(options) {
    ...
    getLocationContent: function (holder) {
        ...
        var tmpText;
        if(!!self.matrikkelInfo ){
          tmpText = '<h4>Velg riktig veg: </h4>';
          var tmpHtml = '';
          for (var i = 0; i < self.matrikkelInfo.length; i++) {
            tmpHtml = tmpHtml + '<option value="'+self.matrikkelInfo[i].veg +' ' + OpenLayers.Lang.translate('in') + ' '  +  self.matrikkelInfo[i].kommune +'">' + self.matrikkelInfo[i].veg +' ' + OpenLayers.Lang.translate('in') + ' '  +  self.matrikkelInfo[i].kommune + '</option>';
          }
          streetAccept.innerHTML = tmpHtml;
        } else {
          tmpText = '<h4>Ingen veg funnet.</h4>';
        }
        streetAcceptContainer.appendChild(streetAccept);
        tmpText += '' + streetAcceptContainer.outerHTML;
    ...
    */
    ...
    setStreetName: function(){
      ...
      this.matrikkelInfo = this.matrikkelInfo[this.streetNameAccept];
      return !(this.streetNameAccept=== undefined || this.streetNameAccept=== null);
    },
