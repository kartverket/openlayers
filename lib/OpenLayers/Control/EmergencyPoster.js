    ...
    */
                    ...
                    '<div class="location-details-wrapper">'+
                        ...
                        '</div>'+
                          '<p>' + OpenLayers.Lang.translate('My UTM location') + ' Sone ' + sone + ' ' + Math.round(localCoo.lon)+',&nbsp;'+Math.round(localCoo.lat) +
                            '</br>' + 'Min posisjon i desimalgrader: N' + md.lat +  ' - E' + md.lon +
                          '</p>'+
    ...
    getLocationContent: function (holder) {
        ...
        if(!!self.matrikkelInfo.veg ){
          tmpText = 'Funnet veg er: ';
          streetInfo = self.matrikkelInfo.veg;
        } else {
          tmpText = 'Ingen veg funnet.';
        }
        var title;
        title = (!self.pointData.stedsnavn || self.pointData.stedsnavn.length <= 1) ? OpenLayers.Lang.translate('Unknown') : self.pointData.stedsnavn;
        ...
        if (self.pointData.kommune) {
            ...
            streetInfo += ' ' + OpenLayers.Lang.translate('in') + ' '  +  self.pointData.kommune +  ' ' + OpenLayers.Lang.translate('municipality');
        }
        ...
        container.innerHTML =
            ...
            '<div class="step-body">' +
                ...
                '<div class="emergency-point-location-wrapper">' +
                    ...
                    '<h2>' + tmpText + '</h2>' +
                    '<h3>' + streetInfo + '</h3>' + streetAcceptContainer.outerHTML +
                '</div>' +
            '</div>';
