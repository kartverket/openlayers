...
OpenLayers.Control.EmergencyPoster = OpenLayers.Class(OpenLayers.Control.Panel, {
                ...
                '<div class="preview">'+
                    ...
                    '<div class="location-details-wrapper">'+
                        '<div class="location-details-header">'+
                            ...
                            '<h2>'+self.pointData.stedsnavn+' '+OpenLayers.Lang.translate('in')+' '+self.pointData.kommune+' kommune</h2>'+ // TODO
    ...
    getLocationContent: function (holder) {
        ...
        var placename = (self.pointData.stedsnavn.length > 1) ? self.pointData.stedsnavn : OpenLayers.Lang.translate('Unknown');
    ...
    */
    ...
    getPointData: function () {
        ...
        */
        ...
        rect   = makeSquare(lonLat.lon, lonLat.lat, 1);
    ...
    buildAltPointsList: function(){
        ...
        for (var i = 0; i < self.altPoints.length; i++) {
            if (html) {
                ...
                html = '<option disabled>Velg annet sted:</option>';
            }
        }
    ...
    setPointData: function(ssrid) {
        ...
        for (var i = 0; i < self.altPoints.length; i++) {
            if (ssrid == self.altPoints[i].ssrID) {
                self.pointData = self.altPoints[i];
                //self.coordinates = new OpenLayers.LonLat(self.pointData.lon, self.pointData.lat).transform(new OpenLayers.Projection('EPSG:'+self.pointData.epsgKode), this.map.getProjectionObject());
            }
        }
    },
