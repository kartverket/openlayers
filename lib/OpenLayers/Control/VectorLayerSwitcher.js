...
OpenLayers.Control.VectorLayerSwitcher = 
	...
	self.toggleWidgetDiv.addEventListener( 'click', function( e ) {
	    ...
	    var has    = self.hasClass(parent,'show');
	    self.toggleClass( parent, 'show' ); 
	    if ( ! has ) {
		self.removeClass( parent, 'displayHorizontal' );
		var wSize = self.getWindowSize();
		if ( ! self.height ) {
		    var offset  = self.getOffset( self.widget ); 
		    var height  = self.widget.offsetHeight;		    
		    self.height = offset['top'] + height;		    
		}

		if ( wSize['height'] < self.height )
		    self.addClass( parent, 'displayHorizontal' ); 
	    }
	}, true );
	...
	window.addEventListener('resize', function() {
		...
		var parent = self.toggleWidgetDiv.parentNode;  
		if ( self.hasClass(parent,'show') && self.height ) {
		    var wSize = self.getWindowSize();
		    wSize['height'] < self.height ?
			self.addClass( parent, 'displayHorizontal' ) :
			self.removeClass( parent, 'displayHorizontal' ); 
		}
	    }, 200 );
